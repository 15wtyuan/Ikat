        ...
        let rn = match &n.kind {
            ...
            NodeKind::RichText { runs } => {
                ...
                for (img_idx, img) in layout.images.iter().enumerate() {
                    // 顶点空间 = 节点世界：img.x/y 是行内局部坐标，须加 rect.x/y（同字形
                    // build_text_mesh 的 + rect.x/y）。缺 rect 偏移则图跑到 design 原点。
...
fn bake_content_offset(layout: &mut crate::text::layout::TextLayout, off_x: f32, off_y: f32) {
    for line in &mut layout.lines {
        ...
        // 行内图世界 y = line.y + y_top 也依赖 line.y 含 off_y。
        line.y += off_y;
        line.baseline += off_y;
            ...
            if run.deco.underline || run.deco.strike {
                if let Some(x0) = run_x_start {
                    ...
                    let deco_offsets: [Option<f32>; 2] = [
                        ...
                        run.deco
                            .strike
                            .then_some(line.baseline - run.font_size * 0.3),
                    ];
                    let x1 = x0 + run_w;
...
fn push_text_meshes(
    ...
    for (pi, (page, verts, uvs, colors, indices)) in meshes[1..].iter().enumerate() {
        let sub_id = synth_text_node_id(n.id.0, (pi + 1) as u32);
