...
fn node_sort_keys_filled_for_empty_div_slot() {
    ...
    let nh_stage = scene.find_by_id_attr("nh-stage").expect("nh-stage 节点存在");
    let nh_effect = scene.find_by_id_attr("nh-effect").expect("nh-effect 节点存在");
    ...
    assert!(stage_key > 0, "nh-stage node_sort_keys 非 0（DFS 序号），got {}", stage_key);
    assert!(effect_key > 0, "nh-effect node_sort_keys 非 0（DFS 序号），got {}", effect_key);
    // DFS 序保 DOM 顺序：nh-stage 在 nh-effect 前。
    assert!(stage_key < effect_key,
