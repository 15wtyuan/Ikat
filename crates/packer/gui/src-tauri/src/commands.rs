...
//!
//! build / init 语义走 `yio` CLI 子进程（与 agent 会话同一二进制、同一版本——
//! GUI 不再自带打包语义）；workspace 表单读写仍走库（人类检查 AI 配置的驾驶舱）。
//! 找不到 yio.exe（dev 模式）时降级进程内调用并注明。
...
use yio_pkg::build::{build, BuildReport};
use yio_pkg::config;
use yio_pkg::diag::BuildFailure;
use yio_pkg::init::{init, CliSource, InitOptions};
use yio_pkg::workspace::{load_workspace, save_workspace as write_workspace, Workspace};

/// 打开/新建工作区的返回：workspace + 解析出的 ui 路径。用户给 GUI 的可能是会话
/// 根（`.yio/config.json` 所在），workspace.json 实际在 `ui_root` 指向处——前端
...
/// 工作区生成物（skills + `.yio/` CLI）新旧状态：打开工作区时探测，stale 时前端
/// 亮「Update workspace」。版本基准 = GUI 链接的 yio_pkg 版本（release 双 exe
/// 同 commit 配套，库版本即同目录 yio.exe 版本）。
#[derive(serde::Serialize)]
#[serde(rename_all = "camelCase")]
...
pub fn workspace_update_state(path: String) -> Result<WorkspaceUpdateState, String> {
    ...
    let stamped = fs::read_to_string(
        located
            .root
            .join(".yio")
            .join(yio_pkg::scaffold::VERSION_STAMP),
    )
    .map(|s| s.trim().to_string())
    .unwrap_or_default();
    let current = yio_pkg::scaffold::YIO_VERSION.to_string();
...
/// 「Update workspace」：刷新会话根生成物（三 skill + `.yio/` CLI + 版本戳）。
/// 首选子进程 `yio scaffold`（与 agent 会话同一 exe）；dev 无 exe 时进程内降级
/// （exe 拷贝源 = GUI 同目录 yio，无则跳过拷贝——refresh_workspace 同语义）。
#[tauri::command]
pub fn update_workspace(path: String) -> Result<WorkspaceUpdateState, String> {
    let located = config::locate(Path::new(&path)).map_err(|e| e.message)?;
    if let Some(yio) = locate_yio(Some(&located.root)) {
        let mut cmd = yio_command(&yio);
        cmd.arg("scaffold").current_dir(&located.root);
        let out = run_capture(cmd).map_err(|e| format!("spawn yio scaffold: {e}"))?;
        if !out.status.success() {
            return Err(format!(
                "yio scaffold 失败（exit {}）：{}",
                out.status.code().unwrap_or(-1),
                String::from_utf8_lossy(&out.stderr)
            ));
        }
    } else {
        let agents = yio_pkg::scaffold::detect_agents(&located.root);
        yio_pkg::scaffold::refresh_workspace(&located.root, &agents)
...
pub fn create_workspace(
    ...
    // 首选子进程（与 agent 会话同一 yio.exe；自拷贝语义天然正确）。
    if let Some(yio) = locate_yio(None) {
        let mut cmd = yio_command(&yio);
        cmd.arg("init")
            .arg(root)
            .arg("--ui")
        ...
        let out = run_capture(cmd).map_err(|e| format!("spawn yio init: {e}"))?;
        if !out.status.success() {
            return Err(format!(
                "yio init 失败（exit {}）：{}",
                out.status.code().unwrap_or(-1),
        ...
        // dev fallback：进程内 init；CLI 源用 GUI 同目录的 yio（若有），否则跳过拷贝。
        let cli_source = std::env::current_exe()
            .ok()
            .and_then(|g| g.parent().map(|d| d.join(yio_file_name())))
            .filter(|p| p.is_file())
...
/// 打包走 `yio build` 子进程（stdout JSON → 还原 BuildReport）；失败时 message +
/// 诊断文本化给前端。找不到 yio.exe（dev 模式 target 目录分离）→ 进程内降级。
/// 入参 path 接受会话根或 ui 目录（config 发现统一解析）。
#[tauri::command]
pub fn run_build(path: String) -> Result<BuildReport, String> {
    let located = config::locate(Path::new(&path)).map_err(|e| e.message)?;
    let Some(yio) = locate_yio(Some(&located.root)) else {
        eprintln!("dev fallback: yio.exe not found next to GUI / in .yio/ — in-process build");
    ...
    cmd.arg("build")
        ...
        .arg("--format")
        .arg("json");
    let out = run_capture(cmd).map_err(|e| format!("spawn yio build: {e}"))?;
    let stdout = String::from_utf8_lossy(&out.stdout).into_owned();
    let json: serde_json::Value = serde_json::from_str(&stdout)
        .map_err(|e| format!("parse yio build output: {e}\nstdout: {stdout}"))?;
    if json["success"].as_bool() == Some(true) {
        ...
        let msg = json["message"]
            .as_str()
            .unwrap_or("unknown yio build failure");
...
/// 定位 yio CLI：(1) GUI 同目录（release = Editor/Tools 双 exe，版本配套）
/// (2) `<workspace>/.yio/`。找不到 = None（dev 模式 target 目录分离，调用方降级）。
fn locate_yio(workspace: Option<&Path>) -> Option<PathBuf> {
    let name = yio_file_name();
    ...
    if let Some(ws) = workspace {
        let bundled = ws.join(".yio").join(&name);
        if bundled.is_file() {
            return Some(bundled);
        }
    }
...
/// 构造 yio 子进程命令（Windows 下 CREATE_NO_WINDOW 防 GUI 黑窗闪烁）。
fn yio_command(exe: &Path) -> Command {
...
mod tests {
    ...
    fn temp_root(tag: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("yio_gui_test_{tag}_{}", std::process::id()));
    ...
    fn scaffold_writes_skills_only_no_instruction_doc() {
        ...
        yio_pkg::scaffold::write_agent_scaffold(
            &root,
        ...
        for skills in [".claude/skills", ".agents/skills"] {
            assert!(
                root.join(skills).join("yio-editor/SKILL.md").is_file(),
                "{skills} 的 editor skill 须落位"
            );
            assert!(
                root.join(skills)
                    .join("yio-editor/references/patterns.md")
                    .is_file(),
                "{skills} 的 editor references 须落位"
            );
            assert!(
                root.join(skills).join("yio-runtime/SKILL.md").is_file(),
                "{skills} 的 runtime skill 须落位"
            );
            assert!(root.join(skills).join("yio/SKILL.md").is_file());
        }
        // 不生成指令文档（AGENTS.md / CLAUDE.md 由用户自持）。
        assert!(!root.join("AGENTS.md").exists());
        assert!(!root.join("CLAUDE.md").exists());

        assert!(yio_pkg::scaffold::write_agent_scaffold(&root, &[]).is_err());
        assert!(yio_pkg::scaffold::write_agent_scaffold(&root, &["cursor".to_string()]).is_err());
