...
//! 按各自工程惯例决定目录，如 Unity 传 <Project>/UserSettings/LoomGUI），
//! 双击 exe 等无参启动不落盘。
...
mod tests {
    ...
    fn parse_state_dir_arg() {
        ...
        assert_eq!(
            state_dir_from_args(&f(&["exe", "--state-dir", "C:/proj/UserSettings/LoomGUI"])),
            Some(PathBuf::from("C:/proj/UserSettings/LoomGUI"))
        );
    ...
    fn disk_roundtrip_push_load_remove() {
        let dir = std::env::temp_dir().join(format!("loomgui_recent_test_{}", std::process::id()));
