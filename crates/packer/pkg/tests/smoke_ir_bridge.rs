...
fn smoke_control_kinds_load_without_crash() {
    ...
    let html = r#"<!DOCTYPE html><html><head><style>input[type="range"],input[type="checkbox"],input[type="radio"]{width:80px}</style></head><body><div style="display:flex"><input type="text"><input type="range"><input type="checkbox"><input type="radio"><select><option></option></select></div></body></html>"#;
