//! fence @keyframes → bridge → pkg.bin 往返契约测试（@yio-hook 锚点 + transform TRS 分解）。
//!
...
//! `/* @yio-hook name */` 注释锚点挂在前一个 stop 上、stop 的 transform 按 TRS 分解
//! （translateY(20px) → translate [0,20]，不做矩阵合并）。

use yio_core::asset::read_package;
use yio_core::scene::animation::{AnimatableProps, KeyframeStopSelector, TransformAnim};
use yio_core::scene::LenDomain;
...
@keyframes slideIn{from{opacity:0;transform:translateY(20px)}/* @yio-hook start */ to{opacity:1;transform:none}}
...
fn keyframes_survive_roundtrip_with_hook_and_trs() {
    ...
    // ② @yio-hook 锚点：注释写在 from 块后、to 前 → 挂在前一个 stop（from）上。
    assert_eq!(
    ...
    assert_eq!(
        ...
        AnimatableProps {
            ...
            transform: Some(TransformAnim {
                translate: Some([
                    yio_core::transform::LenPct::ZERO,
                    yio_core::transform::LenPct { px: 20.0, pct: 0.0 },
                ]),
                scale: None,
                rotate: None,
            }),
...
<div class="card">x</div>"#;
    ...
    let pct = |v: f32| yio_core::transform::LenPct { px: 0.0, pct: v };
    assert_eq!(
        kf.stops[0].props.transform.as_ref().unwrap().translate,
        Some([
            pct(-50.0),
            yio_core::transform::LenPct { px: 0.0, pct: 0.0 }
        ]),
        "from translateX(-50%) 进 pkg（不再静默丢）"
    );
    assert_eq!(
        kf.stops[1].props.transform.as_ref().unwrap().translate,
        Some([pct(50.0), yio_core::transform::LenPct { px: 0.0, pct: 0.0 }])
    );
    assert_eq!(
        kf.stops[0].timing,
        Some(yio_core::tween::Ease::BackOut),
