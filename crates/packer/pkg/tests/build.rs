...
use std::path::Path;
use yio_pkg::build::build;
use yio_pkg::runtime::{RuntimeManifest, RUNTIME_FILE};
...
fn write_workspace_json(root: &Path, atlases: &str, fonts: &str) {
    ...
    std::fs::write(root.join("yio.workspace.json"), json).unwrap();
}

#[test]
fn build_e2e_produces_all_artifacts() {
    let tmp = std::env::temp_dir().join("yio_build_e2e_test");
    ...
    let atlas_m: yio_pkg::atlas::AtlasManifest = serde_json::from_str(&atlas_text).unwrap();
    assert!(
    ...
    assert!(rt_path.exists(), "yio.runtime.json exists");
...
fn build_fails_when_font_file_missing() {
    let tmp = std::env::temp_dir().join("yio_build_missing_font_test");
    let _ = std::fs::remove_dir_all(&tmp);
...
fn build_fails_when_output_dir_empty() {
    let tmp = std::env::temp_dir().join("yio_build_no_output_test");
    let _ = std::fs::remove_dir_all(&tmp);
...
fn build_propagates_warnings_into_report() {
    let tmp = std::env::temp_dir().join("yio_build_warnings_test");
    let _ = std::fs::remove_dir_all(&tmp);
...
fn build_e2e_packages_path_writes_pkg_bin_and_fills_runtime() {
    let tmp = std::env::temp_dir().join("yio_build_packages_e2e_test");
    let _ = std::fs::remove_dir_all(&tmp);
    ...
    // yio.runtime.json packages matches report.packages (fill-back ordering correct).
    let rt_path = output.join(RUNTIME_FILE);
    assert!(rt_path.exists(), "yio.runtime.json exists");
    let rt_text = std::fs::read_to_string(&rt_path).unwrap();
    let rt: RuntimeManifest = serde_json::from_str(&rt_text).unwrap();
