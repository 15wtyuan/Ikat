//! `yio init`：初始化工作区——会话根上落 agent skills + CLI 自拷贝 + 接线 config
//! （`.yio/config.json`），ui 目录上落 workspace.json 骨架。
//!
//! 产出即自足：游戏仓库的 agent 会话零安装（`.yio/yio(.exe)` 就地可用），打开
...
pub enum CliSource {
    ...
    CurrentExe,
    /// 从指定路径拷（GUI dev fallback：GUI 同目录的 yio.exe）。
    Explicit(PathBuf),
    /// 不拷（找不到 yio 二进制——工作区靠 PATH / Release 下载位兜底）。
    Skip,
}
...
pub struct InitOptions {
    ...
    /// Unity 工程根：写入 `.yio/config.json` 的 `unity_root`（内部相对化；None = 不写，
    /// 本地输出）。
...
pub fn init(dir: &Path, opts: InitOptions) -> Result<InitOutcome, BuildFailure> {
    ...
    // CLI 自拷贝 + 接线 config + 版本戳（同住根上 .yio/：整个目录入库，团队 clone 即得）。
    let cli_copied = copy_cli_into(&root, &opts.cli_source);
    let _ = std::fs::write(
        root.join(".yio").join(crate::scaffold::VERSION_STAMP),
        crate::scaffold::YIO_VERSION,
    );
...
/// 拷 CLI 二进制到 `<root>/.yio/`。失败（如 exe 被锁）不阻断 init——工作区已可用
///（PATH 里的 yio / Release 下载位是兜底），CLI 缺席由调用方提示。
fn copy_cli_into(root: &Path, source: &CliSource) -> bool {
    ...
    if std::fs::create_dir_all(&yio_dir).is_err() {
        return false;
    }
...
mod tests {
    ...
    /// 分离形态全流程：根上 skills/.yio/config，ui 上 workspace.json；指针相对化。
    #[test]
    fn init_split_layout() {
        let tmp = std::env::temp_dir().join(format!("yio_init_split_{}", std::process::id()));
        ...
        assert!(tmp.join("ui/yio.workspace.json").exists());
        // 会话根：skills + config（无指令文档——AGENTS.md 不再生成）。
        assert!(tmp.join(".agents/skills/yio-editor/SKILL.md").exists());
        assert!(!tmp.join("ui/AGENTS.md").exists());
        assert!(!tmp.join("AGENTS.md").exists());
        let cfg: crate::config::YioConfig = serde_json::from_str(
    ...
    fn init_standalone_layout() {
        let tmp = std::env::temp_dir().join(format!("yio_init_solo_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&tmp);
        let out = init(&tmp, InitOptions::default()).unwrap();
        assert_eq!(out.root, tmp);
        assert_eq!(out.ui, tmp);
        assert!(tmp.join("yio.workspace.json").exists());
        let cfg: crate::config::YioConfig = serde_json::from_str(
            &std::fs::read_to_string(tmp.join(crate::config::CONFIG_FILE)).unwrap(),
        )
        .unwrap();
