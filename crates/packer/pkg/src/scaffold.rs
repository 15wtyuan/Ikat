//! 会话根脚手架：四个 agent skills（yio-editor 围栏规则手册 / yio-runtime
//! 运行时接线手册 / yio-preview 预览模拟手册 / yio CLI 手册），从模板覆盖拷入。
//! CLI `yio init` / `yio scaffold` 与 GUI「新建工作区」共用（模板只此一份，防两端口径
...
/// 加载）。内容镜像随包 C# 签名契约——消费者 agent 查 API 不需要 Yio 源码仓库。
...
fn skill_artifacts() -> Vec<SkillArtifacts> {
    vec![
        ...
        SkillArtifacts {
            dir: "yio",
            files: vec![("SKILL.md", include_str!("../templates/yio/SKILL.md"))],
        },
    ]
}
...
/// yio CLI 版本（`.yio/scaffold.version` 戳与 check 的 stale 比对共用）。
pub const YIO_VERSION: &str = env!("CARGO_PKG_VERSION");
...
pub fn detect_agents(root: &Path) -> Vec<String> {
    ...
    for (dir, name) in [(".agents/skills", "agents"), (".claude/skills", "claude")] {
        if root.join(dir).join("yio-editor").is_dir() {
...
/// `yio scaffold` 的刷新产物摘要。
#[derive(Debug)]
pub struct RefreshOutcome {
    /// 刷新的 agent 目录（.agents/.claude）。
    pub agents: Vec<String>,
    /// `.yio/yio(.exe)` 是否被更新（同文件自跑 = 已一致，跳过）。
    pub cli_updated: bool,
}

/// 刷新会话根的全部「生成物」：三 skill 覆盖重写 + `.yio/yio(.exe)` 自拷贝 +
/// 版本戳。白名单式——`config.json` / `yio.workspace.json` / HTML/CSS/资源一概
/// 不碰。消费端更新流：UPM 更新包（Editor/Tools 新双 exe 落地）→ 跑一次本命令
/// （任意位置的 yio.exe 均可，exe 以运行者为准）。
pub fn refresh_workspace(root: &Path, agents: &[String]) -> Result<RefreshOutcome, String> {
    write_agent_scaffold(root, agents)?;
    let yio_dir = root.join(".yio");
    std::fs::create_dir_all(&yio_dir).map_err(|e| format!("create {}: {e}", yio_dir.display()))?;
    ...
    let cli_updated = match std::env::current_exe() {
        Ok(src) if src.is_file() => {
            let dst = yio_dir.join(src.file_name().unwrap_or_default());
    ...
    std::fs::write(yio_dir.join(VERSION_STAMP), YIO_VERSION)
...
/// 工作区生成物是否落后于本 CLI：从 `dir` 向上找 `.yio/scaffold.version`（最多
/// 两级，对应会话根/ui 两种摆放），与 [`YIO_VERSION`] 比对。None = 一致或无戳
/// （无 `.yio` 的裸工作区不打扰）。
pub fn stale_stamp_warning(dir: &Path) -> Option<String> {
    let mut cur = Some(dir);
    for _ in 0..3 {
        let Some(d) = cur else { break };
        let stamp = d.join(".yio").join(VERSION_STAMP);
        if let Ok(v) = std::fs::read_to_string(&stamp) {
            let v = v.trim();
            if v != YIO_VERSION {
                return Some(format!(
                    "workspace scaffold is from yio {v} but this yio is {YIO_VERSION} — \
                     run `yio scaffold` to refresh skills and the pinned CLI",
                ));
            }
            return None;
        }
...
mod tests {
    ...
    fn scaffold_writes_three_skills_per_agent() {
        let tmp = std::env::temp_dir().join(format!("yio_scaffold_test_{}", std::process::id()));
        ...
        for skills in [".claude/skills", ".agents/skills"] {
            assert!(
                tmp.join(skills).join("yio-editor/SKILL.md").is_file(),
                "{skills} 的 editor skill 须落位"
            );
            assert!(
                tmp.join(skills)
                    .join("yio-editor/references/fence-schema.md")
                    .is_file(),
                "{skills} 的 editor references 须落位"
            );
            assert!(
                tmp.join(skills).join("yio-runtime/SKILL.md").is_file(),
                "{skills} 的 runtime skill 须落位"
            );
            assert!(
                tmp.join(skills)
                    .join("yio-runtime/references/api-reference.md")
                    .is_file(),
                "{skills} 的 runtime references 须落位"
            );
            let yio = std::fs::read_to_string(tmp.join(skills).join("yio/SKILL.md")).unwrap();
            assert!(yio.contains("yio check"), "yio skill 须含命令面");
        }
