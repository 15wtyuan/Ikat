...
/// loom CLI 版本（`.loom/scaffold.version` 戳与 check 的 stale 比对共用）。
...
pub fn detect_agents(root: &Path) -> Vec<String> {
    ...
    for (dir, name) in [(".agents/skills", "agents"), (".claude/skills", "claude")] {
        if root.join(dir).join("loomgui-editor").is_dir() {
...
/// `loom scaffold` 的刷新产物摘要。
#[derive(Debug)]
pub struct RefreshOutcome {
    /// 刷新的 agent 目录（.agents/.claude）。
    pub agents: Vec<String>,
    /// `.loom/loom(.exe)` 是否被更新（同文件自跑 = 已一致，跳过）。
    pub cli_updated: bool,
}

/// 刷新会话根的全部「生成物」：三 skill 覆盖重写 + `.loom/loom(.exe)` 自拷贝 +
/// 版本戳。白名单式——`config.json` / `loom.workspace.json` / HTML/CSS/资源一概
/// 不碰。消费端更新流：UPM 更新包（Editor/Tools 新双 exe 落地）→ 跑一次本命令
/// （任意位置的 loom.exe 均可，exe 以运行者为准）。
pub fn refresh_workspace(root: &Path, agents: &[String]) -> Result<RefreshOutcome, String> {
    ...
    std::fs::create_dir_all(&loom_dir)
        .map_err(|e| format!("create {}: {e}", loom_dir.display()))?;
    ...
    std::fs::write(loom_dir.join(VERSION_STAMP), LOOM_VERSION)
...
/// 工作区生成物是否落后于本 CLI：从 `dir` 向上找 `.loom/scaffold.version`（最多
/// 两级，对应会话根/ui 两种摆放），与 [`LOOM_VERSION`] 比对。None = 一致或无戳
/// （无 `.loom` 的裸工作区不打扰）。
pub fn stale_stamp_warning(dir: &Path) -> Option<String> {
    let mut cur = Some(dir);
    for _ in 0..3 {
        let Some(d) = cur else { break };
        let stamp = d.join(".loom").join(VERSION_STAMP);
        if let Ok(v) = std::fs::read_to_string(&stamp) {
            let v = v.trim();
            if v != LOOM_VERSION {
                return Some(format!(
                    "workspace scaffold is from loom {v} but this loom is {LOOM_VERSION} — \
                     run `loom scaffold` to refresh skills and the pinned CLI",
                ));
            }
            return None;
        }
