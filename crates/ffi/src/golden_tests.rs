...
//!
//! 更新（改 blob 布局 / 改本场景后）：
//! `LOOMGUI_UPDATE_GOLDEN=1 cargo test -p loomgui_ffi_c --lib golden`
//!
...
use loomgui_core::scene::node::{ControlState, NodeId};
...
fn build_golden_bytes() -> (Vec<u8>, Vec<u8>) {
    ...
    // 判真，锁不住矩阵列）。
    let rc = loomgui_stage_set_transform(h, mover, 24.0, 16.0, 1.15, 1.15, 0.0, 0.0, 0.0);
    ...
    let rc = loomgui_stage_append_child(h, clipper, clipped);
    ...
    for child in [title, img, grad, shadow, faded, mover, clipper, bar] {
        let rc = loomgui_stage_append_child(h, root, child);
    ...
    let frame_ptr = loomgui_stage_borrow_frame(h, &mut frame_len);
    ...
    loomgui_stage_set_input(h, &up, 1);
    loomgui_stage_tick(h, 0.016);

    let mut event_count = 0usize;
    let events_ptr = loomgui_stage_borrow_events(h, &mut event_count);
...
fn golden_files_match_current_blob_version() {
    let frame = std::fs::read(format!("{GOLDEN_DIR}/frame-blob.bin"))
        .expect("golden frame-blob.bin 缺失——跑 LOOMGUI_UPDATE_GOLDEN=1 cargo test -p loomgui_ffi_c --lib golden 生成");
    assert!(frame.len() > 12);
    assert_eq!(
...
/// 再生成入口：设 `LOOMGUI_UPDATE_GOLDEN=1` 时把当前场景字节落盘。
/// 不设时 no-op（生成机专用，CI 不写仓库文件）。
#[test]
fn write_golden_files_when_env_set() {
    if std::env::var("LOOMGUI_UPDATE_GOLDEN").ok().as_deref() != Some("1") {
        return;
    }
