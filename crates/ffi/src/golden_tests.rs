...
//!
//! 更新（改 blob 布局 / 改本场景后）：
//! `YIOGUI_UPDATE_GOLDEN=1 cargo test -p yio_ffi_c --lib golden`
//!
...
use yio_core::scene::node::{ControlState, NodeId};
...
fn mk_node(h: *mut StageHandle, kind: &str, css: &str) -> u64 {
    let id = yio_stage_create_node(h, kind.as_ptr(), kind.len(), css.as_ptr(), css.len());
    assert_ne!(id, u64::MAX, "create_node({kind}) failed");
    id
}

fn set_text(h: *mut StageHandle, node: u64, text: &str) {
    let rc = yio_stage_set_text(h, node, text.as_ptr(), text.len());
    assert_eq!(rc, 0, "set_text failed");
}

fn set_src(h: *mut StageHandle, node: u64, src: &str) {
    let rc = yio_stage_set_src(h, node, src.as_ptr(), src.len());
...
fn build_golden_bytes() -> (Vec<u8>, Vec<u8>) {
    ...
    let root = yio_stage_create_root(h, b"div".as_ptr(), 3, root_css.as_ptr(), root_css.len());
    ...
    // 判真，锁不住矩阵列）。
    let rc = yio_stage_set_transform(h, mover, 24.0, 16.0, 1.15, 1.15, 0.0, 0.0, 0.0);
    ...
    let rc = yio_stage_append_child(h, clipper, clipped);
    ...
    for child in [title, img, grad, shadow, faded, mover, clipper, bar] {
        let rc = yio_stage_append_child(h, root, child);
    ...
    let frame_ptr = yio_stage_borrow_frame(h, &mut frame_len);
    ...
    yio_stage_set_input(h, &up, 1);
    yio_stage_tick(h, 0.016);

    let mut event_count = 0usize;
    let events_ptr = yio_stage_borrow_events(h, &mut event_count);
...
fn golden_files_match_current_blob_version() {
    let frame = std::fs::read(format!("{GOLDEN_DIR}/frame-blob.bin"))
        .expect("golden frame-blob.bin 缺失——跑 YIOGUI_UPDATE_GOLDEN=1 cargo test -p yio_ffi_c --lib golden 生成");
    assert!(frame.len() > 12);
    assert_eq!(
...
/// 再生成入口：设 `YIOGUI_UPDATE_GOLDEN=1` 时把当前场景字节落盘。
/// 不设时 no-op（生成机专用，CI 不写仓库文件）。
#[test]
fn write_golden_files_when_env_set() {
    if std::env::var("YIOGUI_UPDATE_GOLDEN").ok().as_deref() != Some("1") {
        return;
    }
