...
use yio_core::scene::node::{ControlState, EditState, NodeKind};
use yio_core::scene::NodeId;
use yio_core::style::resolved::DisplayMode;
...
fn make_progress_stage(value: f32, max: f32) -> (*mut StageHandle, u64) {
    ...
    let root = yio_stage_create_root(h, b"div".as_ptr(), 3, b"".as_ptr(), 0);
...
fn version_returns_c_string() {
    unsafe {
        let s = CStr::from_ptr(yio_version() as *const i8);
...
fn ffi_get_node_computed_style_div() {
    ...
    let rc = yio_stage_get_node_computed_style(h, root, &mut repr);
    assert_eq!(rc, 0, "computed style rc");
    assert_eq!(repr.opacity, 1.0);
    assert_eq!(repr.display_mode, DisplayMode::Block as u8);
    // 无效 node → rc 非 0。
    let rc_bad = yio_stage_get_node_computed_style(h, u64::MAX, &mut repr);
    assert_ne!(rc_bad, 0);
    yio_stage_free(h);
}
...
fn ffi_null_out_is_error() {
    let h = stage_new_with_dejavu(100.0, 100.0);
    let root = yio_stage_create_root(h, b"div".as_ptr(), 3, b"".as_ptr(), 0);
    let rc = yio_stage_get_node_kind(h, root, std::ptr::null_mut());
    assert_ne!(
        rc, 0,
        "get_node_kind: null out + existing node must not return 0"
    );
    let rc_c = yio_stage_get_node_computed_style(h, root, std::ptr::null_mut());
    assert_ne!(
        rc_c, 0,
...
fn create_root_null_css_does_not_ub() {
    let h = stage_new_with_dejavu(100.0, 100.0);
    let root = yio_stage_create_root(h, b"div".as_ptr(), 3, std::ptr::null(), 0);
    assert_ne!(
        root,
        u64::MAX,
...
fn create_node_null_css_does_not_ub() {
    let h = stage_new_with_dejavu(100.0, 100.0);
    let node = yio_stage_create_node(h, b"div".as_ptr(), 3, std::ptr::null(), 0);
    assert_ne!(
        node,
        u64::MAX,
...
fn set_inline_override_null_css_does_not_ub() {
    let h = stage_new_with_dejavu(100.0, 100.0);
    let root = yio_stage_create_root(h, b"div".as_ptr(), 3, b"".as_ptr(), 0);
    let rc = yio_stage_set_inline_override(h, root, std::ptr::null(), 0);
    assert_eq!(
        rc, 0,
...
fn set_text_null_does_not_ub() {
    let h = stage_new_with_dejavu(100.0, 100.0);
    let text = yio_stage_create_node(h, b"span".as_ptr(), 4, std::ptr::null(), 0);
    assert_ne!(text, u64::MAX, "create span ok");
    let rc = yio_stage_set_text(h, text, std::ptr::null(), 0);
    assert_eq!(
        rc, 0,
...
fn ffi_set_get_control_value_progress() {
    ...
    // 合法区间：90 → 90
    let rc = yio_stage_set_control_value(h, node, 90.0);
    assert_eq!(rc, 0, "set_control_value(90) rc");
    let mut out = 0.0f32;
    let rc = yio_stage_get_control_value(h, node, &mut out);
    assert_eq!(rc, 0, "get_control_value rc");
    assert!((out - 90.0).abs() < 0.001, "value == 90, got {out}");
    // 超 max：150 → clamp 100
    let rc = yio_stage_set_control_value(h, node, 150.0);
    ...
    // 负值：-10 → clamp 0
    let rc = yio_stage_set_control_value(h, node, -10.0);
    assert_eq!(rc, 0);
    let rc = yio_stage_get_control_value(h, node, &mut out);
    assert_eq!(rc, 0);
...
fn ffi_set_get_control_value_slider() {
    ...
    let rc = yio_stage_set_control_value(h, node, 83.0);
    ...
    // 超 max clamp
    let rc = yio_stage_set_control_value(h, node, 200.0);
    assert_eq!(rc, 0);
    let rc = yio_stage_get_control_value(h, node, &mut out);
    assert_eq!(rc, 0);
    assert!(
...
fn ffi_control_value_non_value_control_err() {
    ...
    let rc = yio_stage_set_control_value(h, node, 50.0);
...
fn ffi_get_control_value_null_out_err() {
    ...
    let rc = yio_stage_get_control_value(h, node, std::ptr::null_mut());
...
fn ffi_set_get_control_checked_toggle() {
    let (h, node) = make_toggle_stage(false);
    let rc = yio_stage_set_control_checked(h, node, true);
    assert_eq!(rc, 0, "set_control_checked(true) rc");
    let mut out = false;
    let rc = yio_stage_get_control_checked(h, node, &mut out);
    assert_eq!(rc, 0);
    assert!(out, "checked == true");
    // 翻回 false
    let rc = yio_stage_set_control_checked(h, node, false);
    assert_eq!(rc, 0);
    let rc = yio_stage_get_control_checked(h, node, &mut out);
    assert_eq!(rc, 0);
...
fn ffi_set_get_control_max_progress() {
    let (h, node) = make_progress_stage(50.0, 100.0);
    let rc = yio_stage_set_control_max(h, node, 200.0);
    assert_eq!(rc, 0, "set_control_max(200) rc");
    let mut out = 0.0f32;
    let rc = yio_stage_get_control_max(h, node, &mut out);
    assert_eq!(rc, 0);
...
fn ffi_set_get_control_min_step_slider() {
    ...
    let rc = yio_stage_set_control_min(h, node, 10.0);
    assert_eq!(rc, 0, "set_control_min(10) rc");
    let mut out = 0.0f32;
    let rc = yio_stage_get_control_min(h, node, &mut out);
    assert_eq!(rc, 0);
    assert!((out - 10.0).abs() < 0.001, "min == 10, got {out}");

    let rc = yio_stage_set_control_step(h, node, 2.0);
    assert_eq!(rc, 0, "set_control_step(2) rc");
    let rc = yio_stage_get_control_step(h, node, &mut out);
    assert_eq!(rc, 0);
...
fn ffi_control_min_step_progress_err() {
    let (h, node) = make_progress_stage(50.0, 100.0);
    assert_eq!(yio_stage_set_control_min(h, node, 10.0), 0);
    assert_eq!(yio_stage_set_control_step(h, node, 2.0), -1);
    let mut out = 99.0f32;
    assert_eq!(yio_stage_get_control_min(h, node, &mut out), 0);
    assert_eq!(out, 10.0);
    assert_eq!(yio_stage_get_control_step(h, node, &mut out), -1);
    yio_stage_free(h);
}
...
fn ffi_set_transform_translates_user_transform() {
    ...
    let root = yio_stage_create_root(h, b"div".as_ptr(), 3, b"".as_ptr(), 0);
    assert_ne!(root, u64::MAX, "create_root ok");
    let rc = yio_stage_set_transform(h, root, 50.0, 0.0, 1.0, 1.0, 0.0, 0.0, 0.0);
...
fn ffi_set_transform_stores_origin() {
    ...
    let root = yio_stage_create_root(h, b"div".as_ptr(), 3, b"".as_ptr(), 0);
    assert_ne!(root, u64::MAX, "create_root ok");
    let rc = yio_stage_set_transform(h, root, 10.0, 0.0, 1.0, 1.0, 0.0, 5.0, 5.0);
    assert_eq!(rc, 0, "set_transform rc");
...
fn ffi_set_transform_invalid_node_err() {
    let h = stage_new_with_dejavu(200.0, 200.0);
    let rc = yio_stage_set_transform(h, u64::MAX, 10.0, 10.0, 1.0, 1.0, 0.0, 0.0, 0.0);
...
fn ffi_set_control_max_negative_does_not_panic() {
    ...
    let rc = yio_stage_set_control_max(h, node, -5.0);
    ...
    let mut v = -999.0f32;
    let rc = yio_stage_get_control_value(h, node, &mut v);
    assert_eq!(rc, 0);
...
fn ffi_set_control_value_slider_quantize_respects_max() {
    ...
    let rc = yio_stage_set_control_value(h, node, 100.0);
    assert_eq!(rc, 0);
    let mut out = 0.0f32;
    let rc = yio_stage_get_control_value(h, node, &mut out);
    assert_eq!(rc, 0);
...
fn make_stage_with_focused_textfield(value: &str) -> (*mut StageHandle, u64) {
    ...
    let tf = yio_stage_create_node(h, b"div".as_ptr(), 3, b"".as_ptr(), 0);
    assert_ne!(tf, u64::MAX, "create textfield node ok");
    yio_stage_append_child(h, root, tf);
    let sh = unsafe { &mut *h };
    let scene = sh.stage.scene.as_mut().expect("scene built");
    scene.controls.ensure(
        NodeId(tf),
...
fn ffi_text_input_inserts_into_focused_textfield() {
    ...
    assert_eq!(
        yio_stage_set_text_input(h, cps.as_ptr(), 2),
        0,
        "set_text_input rc"
    );
...
fn ffi_text_input_chinese_into_focused_textfield() {
    ...
    yio_stage_tick(h, 0.0); // process_text_input + measure + render
...
fn ffi_set_text_input_null_is_noop() {
    let (h, tf) = make_stage_with_focused_textfield("ab");
    let rc = yio_stage_set_text_input(h, std::ptr::null(), 0);
...
fn ffi_set_text_input_null_handle_err() {
    let rc = yio_stage_set_text_input(std::ptr::null_mut(), std::ptr::null(), 0);
...
fn composition_spliced_into_display() {
    ...
    let s = b"ni";
    let rc = yio_stage_set_composition(h, tf, s.as_ptr(), s.len(), 2);
...
fn commit_composition_appends_to_value() {
    ...
    let s = b"ni";
    assert_eq!(
        yio_stage_set_composition(h, tf, s.as_ptr(), s.len(), 2),
        0,
        "set_composition rc"
    );
    assert_eq!(
        yio_stage_commit_composition(h, tf),
        1,
...
fn get_cursor_rect_returns_finite_rect() {
    ...
    let rc = yio_stage_get_cursor_rect(h, tf, &mut rect);
...
fn composition_ffi_null_handle_err() {
    let rc = yio_stage_set_composition(std::ptr::null_mut(), 0, b"x".as_ptr(), 1, 0);
    assert_eq!(rc, -1, "null handle set_composition -> -1");
    assert_eq!(
        yio_stage_commit_composition(std::ptr::null_mut(), 0),
        -1,
        "null handle commit -> -1"
    );
    assert_eq!(
        yio_stage_get_cursor_rect(std::ptr::null_mut(), 0, std::ptr::null_mut()),
        -1,
...
use std::sync::Mutex;
use yio_core::input::{KeyEvent, EVT_VALUE_CHANGED, MOD_CTRL};
...
fn ffi_clip_setup() -> std::sync::MutexGuard<'static, ()> {
    ...
    *FFI_TEST_CLIP.lock().unwrap() = String::new();
    yio_register_clipboard(Some(ffi_test_set), Some(ffi_test_get));
    g
}
...
fn send_keydown_tick(h: *mut StageHandle, key_code: u32, modifiers: u8) {
    ...
    yio_stage_set_key_input(h, &ke, 1);
    yio_stage_tick(h, 0.0);
}
...
fn ffi_ctrl_c_copies_selection() {
    ...
    send_keydown_tick(h, yio_core::input::KEY_C, MOD_CTRL);
    assert_eq!(
...
fn ffi_ctrl_x_cuts_and_emits_value_changed() {
    ...
    send_keydown_tick(h, yio_core::input::KEY_X, MOD_CTRL);
...
fn ffi_ctrl_v_pastes_and_emits_value_changed() {
    ...
    *FFI_TEST_CLIP.lock().unwrap() = "hi".into();
    ...
    send_keydown_tick(h, yio_core::input::KEY_V, MOD_CTRL);
...
fn ffi_ctrl_a_then_ctrl_c_copies_all() {
    ...
    send_keydown_tick(h, yio_core::input::KEY_A, MOD_CTRL);
    send_keydown_tick(h, yio_core::input::KEY_C, MOD_CTRL);
...
fn ffi_ctrl_v_noop_when_clipboard_unregistered() {
    ...
    yio_register_clipboard(None, None);
    let (h, tf) = make_stage_with_focused_textfield_selection("XY", None);
    send_keydown_tick(h, yio_core::input::KEY_V, MOD_CTRL);
    assert_eq!(
        textfield_value(h, tf),
...
/// yio_register_clipboard 是 process-scoped 全局——测完须清理，防污染其他测试。
...
fn ffi_set_get_control_text_round_trip() {
    ...
    let new = b"new";
    assert_eq!(
        yio_stage_set_control_text(h, tf, new.as_ptr(), new.len()),
        0,
        "set_control_text rc"
    );
    ...
    let mut len = 0usize;
    assert_eq!(
        yio_stage_get_control_text(h, tf, buf.as_mut_ptr(), buf.len(), &mut len),
        0,
...
fn ffi_get_control_text_buf_too_small_returns_needed() {
    ...
    let mut buf = [0u8; 2];
    let mut len = 0usize;
    let rc = yio_stage_get_control_text(h, tf, buf.as_mut_ptr(), buf.len(), &mut len);
...
fn ffi_set_selection() {
    let (h, tf) = make_stage_with_focused_textfield("hello");
    // 选 "el"（字节 [1,3)）。
    assert_eq!(yio_stage_set_selection(h, tf, 1, 3), 0, "set_selection rc");
...
fn ffi_get_selection() {
    ...
    assert_eq!(yio_stage_set_selection(h, tf, 4, 1), 0);
    let mut start = 0usize;
    let mut end = 0usize;
    assert_eq!(yio_stage_get_selection(h, tf, &mut start, &mut end), 0);
    assert_eq!(
        (start, end),
        (1, 4),
...
fn ffi_set_get_control_placeholder() {
    ...
    let ph = b"type here";
    assert_eq!(
        yio_stage_set_control_placeholder(h, tf, ph.as_ptr(), ph.len()),
        0,
    ...
    let mut len = 0usize;
    assert_eq!(
        yio_stage_get_control_placeholder(h, tf, buf.as_mut_ptr(), buf.len(), &mut len),
        0
    );
...
fn ffi_set_control_readonly() {
    let (h, tf) = make_stage_with_focused_textfield("ab");
    assert_eq!(
        yio_stage_set_control_readonly(h, tf, true),
        0,
        "set readonly"
    );
    let sh = unsafe { &*h };
...
fn ffi_set_control_readonly_numberfield() {
    ...
    let rc = yio_stage_set_control_readonly(h, n, true);
    assert_eq!(rc, 0, "set readonly on NumberField returns 0");
    // 读回：get_control_readonly 应返 0 且 out=1
    let mut out: u8 = 9;
    let rc = yio_stage_get_control_readonly(h, n, &mut out);
    ...
    assert_eq!(
        yio_stage_set_control_readonly(h, s, true),
        -1,
...
fn ffi_set_control_maxlength() {
    let (h, tf) = make_stage_with_focused_textfield("ab");
    assert_eq!(
        yio_stage_set_control_maxlength(h, tf, 5),
        0,
        "set maxlength=5"
    );
    let sh = unsafe { &*h };
...
fn ffi_set_control_text_non_text_control_err() {
    ...
    assert_eq!(
        yio_stage_set_control_text(h, root, b"x".as_ptr(), 1),
        -1,
...
fn ffi_control_text_null_handle_err() {
    assert_eq!(
        yio_stage_set_control_text(std::ptr::null_mut(), 0, b"x".as_ptr(), 1),
        -1
    );
    let mut len = 0usize;
    assert_eq!(
        yio_stage_get_control_text(std::ptr::null(), 0, std::ptr::null_mut(), 0, &mut len),
        -1
    );
}
...
fn make_stage_with_textarea(value: &str) -> (*mut StageHandle, u64) {
    ...
    let root = yio_stage_create_root(h, b"div".as_ptr(), 3, b"".as_ptr(), 0);
    assert_ne!(root, u64::MAX, "create_root ok");
    let ta = yio_stage_create_node(h, b"div".as_ptr(), 3, b"".as_ptr(), 0);
    assert_ne!(ta, u64::MAX, "create textarea node ok");
    yio_stage_append_child(h, root, ta);
    let sh = unsafe { &mut *h };
    let scene = sh.stage.scene.as_mut().expect("scene built");
    scene.controls.ensure(
        NodeId(ta),
...
fn ffi_set_control_text_preserves_textarea_variant() {
    ...
    assert_eq!(
        yio_stage_set_control_text(h, ta, b"new".as_ptr(), 3),
        0,
        "set_control_text rc"
    );
    let sh = unsafe { &*h };
...
fn ffi_set_selection_preserves_textarea_variant() {
    ...
    assert_eq!(yio_stage_set_selection(h, ta, 1, 3), 0, "set_selection rc");
    let sh = unsafe { &*h };
    let scene = sh.stage.scene.as_ref().expect("scene built");
    match scene.controls.get(NodeId(ta)) {
        Some(ControlState::TextArea(e)) => {
...
fn make_dropdown_stage(selected: usize, open: bool) -> (*mut StageHandle, u64) {
    ...
    let root = yio_stage_create_root(h, b"div".as_ptr(), 3, b"".as_ptr(), 0);
    assert_ne!(root, u64::MAX, "create_root ok");
    let node = yio_stage_create_node(h, b"div".as_ptr(), 3, b"".as_ptr(), 0);
    assert_ne!(node, u64::MAX, "create dropdown node ok");
    yio_stage_append_child(h, root, node);
    let sh = unsafe { &mut *h };
    let scene = sh.stage.scene.as_mut().expect("scene built");
    scene.controls.ensure(
        NodeId(node),
...
fn ffi_get_node_disabled_reads_flag() {
    ...
    // 默认非 disabled
    let mut out: u8 = 9;
    yio_stage_get_node_disabled(h, root, &mut out);
    assert_eq!(out, 0, "default not disabled");
    // set disabled 后读回 1
    yio_stage_set_node_disabled(h, root, true);
    let mut out: u8 = 9;
    yio_stage_get_node_disabled(h, root, &mut out);
    assert_eq!(out, 1, "disabled flag read back");
    // 无效节点 → 0（false）
    let mut out: u8 = 9;
    yio_stage_get_node_disabled(h, u64::MAX, &mut out);
...
fn ffi_get_control_readonly_text_and_number() {
    ...
    let mut out: u8 = 9;
    let rc = yio_stage_get_control_readonly(h, tf, &mut out);
    ...
    let (h, s) = make_slider_stage(50.0, 0.0, 100.0, 1.0);
    let mut out: u8 = 9;
    let rc = yio_stage_get_control_readonly(h, s, &mut out);
...
fn ffi_blur_sets_pending_none() {
    let h = stage_new_with_dejavu(200.0, 100.0);
    let root = yio_stage_create_root(h, b"div".as_ptr(), 3, b"".as_ptr(), 0);
    yio_stage_request_focus(h, root);
    // blur FFI 包装
    let rc = yio_stage_blur(h);
...
fn ffi_dropdown_selected_index_roundtrip() {
    ...
    let mut idx: u32 = 99;
    let rc = yio_stage_get_dropdown_selected_index(h, dd, &mut idx);
    assert_eq!(rc, 0, "get rc");
    assert_eq!(idx, 1, "initial selected_index");
    // set 3
    let rc = yio_stage_set_dropdown_selected_index(h, dd, 3);
    ...
    let mut idx: u32 = 0;
    assert_eq!(yio_stage_get_dropdown_selected_index(h2, s, &mut idx), -1);
    assert_eq!(yio_stage_set_dropdown_selected_index(h2, s, 0), -1);
    yio_stage_free(h);
    yio_stage_free(h2);
}
...
fn ffi_dropdown_open_roundtrip() {
    ...
    let mut open: u8 = 9;
    let rc = yio_stage_get_dropdown_open(h, dd, &mut open);
    assert_eq!(rc, 0, "get rc");
    assert_eq!(open, 0, "initial closed");
    let rc = yio_stage_set_dropdown_open(h, dd, 1);
    assert_eq!(rc, 0, "set rc");
    let mut open: u8 = 9;
    let rc = yio_stage_get_dropdown_open(h, dd, &mut open);
    assert_eq!(rc, 0);
    ...
    let mut open: u8 = 0;
    assert_eq!(yio_stage_get_dropdown_open(h2, t, &mut open), -1);
    assert_eq!(yio_stage_set_dropdown_open(h2, t, 1), -1);
    yio_stage_free(h);
    yio_stage_free(h2);
}
...
fn ffi_tablist_selected_index_roundtrip() {
    ...
    let mut idx: u32 = 99;
    let rc = yio_stage_get_tablist_selected_index(h, tl, &mut idx);
    assert_eq!(rc, 0, "get rc");
    assert_eq!(idx, 0, "initial selected_index");
    // set 2
    let rc = yio_stage_set_tablist_selected_index(h, tl, 2);
    assert_eq!(rc, 0, "set rc");
    // 读回
    let mut idx: u32 = 99;
    let rc = yio_stage_get_tablist_selected_index(h, tl, &mut idx);
    ...
    let (h2, s) = make_slider_stage(50.0, 0.0, 100.0, 1.0);
    let mut idx: u32 = 0;
    assert_eq!(yio_stage_get_tablist_selected_index(h2, s, &mut idx), -1);
    assert_eq!(yio_stage_set_tablist_selected_index(h2, s, 0), -1);
    yio_stage_free(h);
    yio_stage_free(h2);
}
...
fn ffi_number_value_clamp_and_quantize() {
    ...
    let rc = yio_stage_set_number_value(h, n, 7.0);
    assert_eq!(rc, 0, "set rc");
    let mut out = -1.0f32;
    let rc = yio_stage_get_number_value(h, n, &mut out);
    assert_eq!(rc, 0, "get rc");
    assert!(
        (out - 8.0).abs() < 0.001,
    ...
    // 超 max clamp：15 → 10
    let rc = yio_stage_set_number_value(h, n, 15.0);
    assert_eq!(rc, 0);
    let mut out = -1.0f32;
    let rc = yio_stage_get_number_value(h, n, &mut out);
    assert_eq!(rc, 0);
...
fn ffi_number_value_non_number_control_err() {
    ...
    let mut out = -1.0f32;
    assert_eq!(
        yio_stage_get_number_value(h, s, &mut out),
        -1,
        "get on slider"
    );
    assert_eq!(yio_stage_set_number_value(h, s, 5.0), -1, "set on slider");
    yio_stage_free(h);
}
...
fn ffi_play_animation_creates_programmatic_player() {
    ...
    let key = yio_stage_play_animation(h, root, b"fadeIn".as_ptr(), 6);
    assert_ne!(key, 0, "play returns valid PlayerKey");
    let sh = unsafe { &mut *h };
    let scene = sh.stage.scene.as_mut().unwrap();
    let p = scene
        .players
        .get(yio_core::scene::animation::player_key_from_u64(key))
    ...
    assert_eq!(p.node, yio_core::scene::NodeId(root));
    // 首帧立即写（spec §5.2）：fill both → progress 0 采样 opacity 0。
    let a = scene
        .anim
        .get(yio_core::scene::NodeId(root))
        .expect("first frame written");
    assert_eq!(a.opacity, Some(0.0), "first frame opacity");
    // 状态 Playing。
    assert_eq!(
        yio_stage_get_animation_state(h, key),
        0,
        "state == Playing(0)"
    );
    // 未知 name → 0。
    assert_eq!(
        yio_stage_play_animation(h, root, b"nope".as_ptr(), 4),
        0,
        "unknown name"
    );
    // 无效节点 → 0。
    assert_eq!(
        yio_stage_play_animation(h, u64::MAX, b"fadeIn".as_ptr(), 6),
        0,
        "invalid node"
    );
    // null name 指针 → 0（from_raw_parts(null,..) UB 防御）。
    assert_eq!(
        yio_stage_play_animation(h, root, std::ptr::null(), 0),
        0,
        "null name"
    );
    yio_stage_free(h);
...
fn ffi_pause_resume_animation_state() {
    ...
    let key = yio_stage_play_animation(h, root, b"fadeIn".as_ptr(), 6);
    assert_ne!(key, 0);
    yio_stage_pause_animation(h, key);
    assert_eq!(yio_stage_get_animation_state(h, key), 1, "paused");
    yio_stage_pause_animation(h, key); // 已 Paused 再 pause：幂等
    assert_eq!(yio_stage_get_animation_state(h, key), 1);
    yio_stage_resume_animation(h, key);
    assert_eq!(yio_stage_get_animation_state(h, key), 0, "resumed");
    // 无效 key 全部 no-op（不 panic）。
    yio_stage_pause_animation(h, 0);
    yio_stage_resume_animation(h, 0xDEAD_BEEF);
    yio_stage_free(h);
}
...
fn ffi_set_get_animation_time_roundtrip() {
    ...
    assert_ne!(key, 0);
    assert_eq!(
        yio_stage_get_animation_time(h, key),
        0.0,
        "fresh player time 0"
    );
    yio_stage_set_animation_time(h, key, 0.25);
    assert_eq!(yio_stage_get_animation_time(h, key), 0.25, "seek roundtrip");
    // 无效 key：get → 0.0；set → no-op（无 panic）。
    assert_eq!(
        yio_stage_get_animation_time(h, 0),
        0.0,
        "invalid key time 0"
    );
    yio_stage_set_animation_time(h, 0xDEAD_BEEF, 0.5);
    yio_stage_free(h);
}
...
fn ffi_stop_animation_is_terminal() {
    ...
    let key = yio_stage_play_animation(h, root, b"fadeIn".as_ptr(), 6);
    assert_ne!(key, 0);
    yio_stage_stop_animation(h, key);
    assert_eq!(
        yio_stage_get_animation_state(h, key),
        255,
        "stopped == invalid(255)"
    );
    yio_stage_resume_animation(h, key); // Stopped 不可恢复
    assert_eq!(yio_stage_get_animation_state(h, key), 255);
    ...
    assert_eq!(yio_stage_get_animation_time(h, key), 0.0, "dead key time 0");
    yio_stage_free(h);
}
...
fn ffi_animation_state_completed_after_duration() {
    ...
    let key = yio_stage_play_animation(h, root, b"fadeIn".as_ptr(), 6);
    assert_ne!(key, 0);
    yio_stage_tick(h, 1.1); // 越过 1s 时长
    assert_eq!(yio_stage_get_animation_state(h, key), 2, "completed");
    let sh = unsafe { &mut *h };
    let scene = sh.stage.scene.as_mut().unwrap();
...
fn ffi_animation_on_key_registers_dedup() {
    ...
    let key = yio_stage_play_animation(h, root, b"fadeIn".as_ptr(), 6);
    assert_ne!(key, 0);
    yio_stage_animation_on_key(h, key, 0.5);
    yio_stage_animation_on_key(h, key, 0.5); // 去重
    let sh = unsafe { &mut *h };
    let scene = sh.stage.scene.as_mut().unwrap();
    let p = scene
        .players
    ...
    yio_stage_animation_on_key(h, 0, 0.5);
    yio_stage_free(h);
}
...
fn ffi_get_event_string_reads_animation_name() {
    ...
    yio_stage_tick(h, 0.016);
    let evs = drain_events(h);
    let start = evs
        .iter()
        .find(|e| e.event_type == yio_core::event::EVT_ANIMATION_START)
    ...
    let rc = yio_stage_get_event_string(h, idx, std::ptr::null_mut(), 0, &mut needed);
    assert_eq!(rc, -2, "probe returns -2");
    assert_eq!(needed, b"fadeIn".len(), "probe reports needed len");
    // 真读。
    let mut buf = vec![0u8; needed];
    let rc = yio_stage_get_event_string(h, idx, buf.as_mut_ptr(), buf.len(), &mut needed);
    assert_eq!(rc, 0, "read ok");
    assert_eq!(&buf[..needed], b"fadeIn", "string round-trip");
    // 索引越界 → -1（防御分支）。
    let mut n = 0usize;
    let rc = yio_stage_get_event_string(h, u32::MAX, std::ptr::null_mut(), 0, &mut n);
    assert_eq!(rc, -1, "out-of-range index");
    assert_eq!(n, 0);
    // null 句柄 / null out_len → -1。
    let rc = yio_stage_get_event_string(std::ptr::null(), idx, std::ptr::null_mut(), 0, &mut n);
    assert_eq!(rc, -1, "null handle");
    let rc = yio_stage_get_event_string(h, idx, std::ptr::null_mut(), 0, std::ptr::null_mut());
...
fn ffi_set_control_bounds_numberfield() {
    ...
    assert_eq!(yio_stage_set_control_max(h, n, 4.0), 0);
    let mut out = -999.0f32;
    assert_eq!(yio_stage_get_number_value(h, n, &mut out), 0);
    assert!(
        (out - 4.0).abs() < 0.001,
        "value re-clamped into [0,4], got {out}"
    );
    // max 低于 min → guard 到 min（不 panic，同 Slider arm）。
    assert_eq!(yio_stage_set_control_max(h, n, -1.0), 0);
    assert_eq!(yio_stage_get_control_max(h, n, &mut out), 0);
    assert!((out - 0.0).abs() < 0.001, "max guard to min 0, got {out}");
    // min 0→3：先恢复 max（guard 测试已把 max 压到 0，min 会被 min.min(max) clamp）。
    assert_eq!(yio_stage_set_control_max(h, n, 10.0), 0);
    assert_eq!(yio_stage_set_control_min(h, n, 3.0), 0);
    assert_eq!(yio_stage_get_control_min(h, n, &mut out), 0);
    assert!((out - 3.0).abs() < 0.001, "min stored");
    // step 换 1：合法。
    assert_eq!(yio_stage_set_control_step(h, n, 1.0), 0);
    assert_eq!(yio_stage_get_control_step(h, n, &mut out), 0);
    assert!((out - 1.0).abs() < 0.001, "step stored");
    // 非法 step（负 / NaN）→ -1，不落脏值。
    assert_eq!(yio_stage_set_control_step(h, n, -2.0), -1);
    assert_eq!(yio_stage_set_control_step(h, n, f32::NAN), -1);
    ...
    assert_eq!(yio_stage_set_control_max(h, n, 8.0), 0);
    let mut buf = [0u8; 16];
    let mut len = 0usize;
    assert_eq!(
        yio_stage_get_control_text(h, n, buf.as_mut_ptr(), 16, &mut len),
        -1,
...
fn ffi_progress_indeterminate_roundtrip() {
    ...
    let mut b = 2u8;
    assert_eq!(yio_stage_get_control_indeterminate(h, p, &mut b), 0);
    assert_eq!(b, 0, "初始 determinate");
    assert_eq!(yio_stage_set_control_indeterminate(h, p, 1), 0);
    assert_eq!(yio_stage_get_control_indeterminate(h, p, &mut b), 0);
    assert_eq!(b, 1, "set true → get true");
    assert_eq!(yio_stage_set_control_indeterminate(h, p, 0), 0);
    assert_eq!(yio_stage_get_control_indeterminate(h, p, &mut b), 0);
    assert_eq!(b, 0, "set false → get false");
    // value/max 不被扰动。
    let mut v = -1.0f32;
    assert_eq!(yio_stage_get_control_value(h, p, &mut v), 0);
    assert!((v - 40.0).abs() < 0.001);
    // 非 Progress（Slider）→ -1。
    let (h2, s) = make_slider_stage(50.0, 0.0, 100.0, 1.0);
    assert_eq!(yio_stage_set_control_indeterminate(h2, s, 1), -1);
    assert_eq!(yio_stage_get_control_indeterminate(h2, s, &mut b), -1);
    yio_stage_free(h);
    yio_stage_free(h2);
}
...
fn ffi_get_radio_name() {
    ...
    let mut needed = 0usize;
    assert_eq!(
        yio_stage_get_radio_name(h, r, std::ptr::null_mut(), 0, &mut needed),
        -2
    );
    ...
    assert_eq!(
        yio_stage_get_radio_name(h, r, buf.as_mut_ptr(), buf.len(), &mut written),
        0
    );
    ...
    let mut n2 = 0usize;
    assert_eq!(
        yio_stage_get_radio_name(h, r, cap_buf.as_mut_ptr(), 10, &mut n2),
        0
    );
    assert_eq!(n2, 10);
    ...
    let mut n3 = 99usize;
    assert_eq!(
        yio_stage_get_radio_name(h2, r2, std::ptr::null_mut(), 0, &mut n3),
        0,
    ...
    let mut n4 = 0usize;
    assert_eq!(
        yio_stage_get_radio_name(h3, s, std::ptr::null_mut(), 0, &mut n4),
        -1
    );
    yio_stage_free(h);
    yio_stage_free(h2);
...
/// yio_stage_hit_test：坐标命中最上层 touchable 节点（rc=0 + out id）；
...
fn ffi_stage_hit_test_basic() {
    ...
    yio_stage_append_child(h, root, node);
    {
        ...
        r.layout_rect = yio_core::scene::node::Rect {
            x: 0.0,
            y: 0.0,
            w: 200.0,
            h: 100.0,
        ...
        c.layout_rect = yio_core::scene::node::Rect {
            x: 10.0,
            y: 10.0,
            w: 50.0,
            h: 50.0,
        };
        yio_core::scene::transform::compute_world_transforms(scene);
    }
    // 命中子节点。
    let mut out = u64::MAX;
    assert_eq!(yio_stage_hit_test(h, 20.0, 20.0, &mut out), 0);
    assert_eq!(out, node, "子节点区域命中子");
    // 子外 root 区域 → 无命中（文档根是宿主容器，不可命中——多 Stage 输入路由依赖
    // 「空白处 = 无命中」，可命中的全画布根会把底层 Stage 输入饿死）→ rc=1。
    assert_eq!(yio_stage_hit_test(h, 150.0, 90.0, &mut out), 1);
    // 画布外 → rc=1。
    assert_eq!(yio_stage_hit_test(h, 500.0, 500.0, &mut out), 1);
    // null out 指针（防御）→ -1。
    assert_eq!(yio_stage_hit_test(h, 20.0, 20.0, std::ptr::null_mut()), -1);
    // null 句柄 → -1。
    assert_eq!(
        yio_stage_hit_test(std::ptr::null(), 20.0, 20.0, std::ptr::null_mut()),
        -1
    );
    yio_stage_free(h);
}
...
fn ffi_node_touchable_roundtrip_and_hit() {
    ...
    // 初始默认 touchable。
    let mut b = 9u8;
    assert_eq!(yio_stage_get_node_touchable(h, node, &mut b), 0);
    ...
    yio_stage_set_node_touchable(h, node, false);
    assert_eq!(yio_stage_get_node_touchable(h, node, &mut b), 0);
    assert_eq!(b, 0, "untouchable now");
    assert_eq!(yio_stage_hit_test(h, 20.0, 20.0, &mut out), 1);
    assert_eq!(out, node, "no hit（rc=1）不写 out——保留上次命中值");
    // 恢复 → 命中回归。
    yio_stage_set_node_touchable(h, node, true);
    assert_eq!(yio_stage_hit_test(h, 20.0, 20.0, &mut out), 0);
    assert_eq!(out, node, "touchable restored");
    // 越界节点 / null out → -1。
    let mut n2 = 0u8;
    assert_eq!(yio_stage_get_node_touchable(h, u64::MAX, &mut n2), -1);
    assert_eq!(
        yio_stage_get_node_touchable(h, node, std::ptr::null_mut()),
        -1
    );
    yio_stage_free(h);
}
...
fn ffi_node_draggable_roundtrip() {
    ...
    let mut b = 9u8;
    assert_eq!(yio_stage_get_node_draggable(h, node, &mut b), 0);
    assert_eq!(b, 0, "default not draggable");
    // 开 → round-trip 回 1；再关回 0。
    yio_stage_set_node_draggable(h, node, true);
    assert_eq!(yio_stage_get_node_draggable(h, node, &mut b), 0);
    assert_eq!(b, 1, "draggable now");
    yio_stage_set_node_draggable(h, node, false);
    assert_eq!(yio_stage_get_node_draggable(h, node, &mut b), 0);
    assert_eq!(b, 0, "drag disabled again");
    // 越界节点 / null out → -1。
    let mut n2 = 0u8;
    assert_eq!(yio_stage_get_node_draggable(h, u64::MAX, &mut n2), -1);
    assert_eq!(
        yio_stage_get_node_draggable(h, node, std::ptr::null_mut()),
        -1
    );
    // null 句柄 set = no-op（不 panic）。
    yio_stage_set_node_draggable(std::ptr::null_mut(), node, true);
    yio_stage_free(h);
}
...
fn ffi_tab_activation_roundtrip() {
    ...
    let tl = yio_stage_create_node(h, b"div".as_ptr(), 3, b"".as_ptr(), 0);
    assert_ne!(tl, u64::MAX);
    yio_stage_append_child(h, root, tl);
    {
        let sh = unsafe { &mut *h };
        let scene = sh.stage.scene.as_mut().expect("scene built");
        scene.controls.ensure(
            NodeId(tl),
            yio_core::scene::node::ControlState::TabList {
                selected_index: 0,
                manual_activation: false,
            },
        );
    ...
    let mut b = 9u8;
    assert_eq!(yio_stage_get_tab_activation(h, tl, &mut b), 0);
    assert_eq!(b, 0, "default automatic");
    yio_stage_set_tab_activation(h, tl, true);
    assert_eq!(yio_stage_get_tab_activation(h, tl, &mut b), 0);
    assert_eq!(b, 1, "manual now");
    // 非 TabList 节点 → -1。
    let mut n2 = 0u8;
    assert_eq!(yio_stage_get_tab_activation(h, root, &mut n2), -1);
    assert_eq!(yio_stage_set_tab_activation(h, root, true), -1);
    yio_stage_free(h);
}
...
fn ffi_get_custom_tag_two_call() {
    ...
    let mut needed = 0usize;
    assert_eq!(
        yio_stage_get_custom_tag(h, node, std::ptr::null_mut(), 0, &mut needed),
        -2
    );
    ...
    let mut buf = vec![0u8; needed];
    assert_eq!(
        yio_stage_get_custom_tag(h, node, buf.as_mut_ptr(), buf.len(), &mut needed),
        0
    );
    ...
    let mut n2 = 0usize;
    assert_eq!(
        yio_stage_get_custom_tag(h, root, std::ptr::null_mut(), 0, &mut n2),
        -1
    );
    // 越界节点 → -1
    assert_eq!(
        yio_stage_get_custom_tag(h, u64::MAX, std::ptr::null_mut(), 0, &mut n2),
        -1
    );
    yio_stage_free(h);
}

/// yio_node_is_lookup_scope：实例根 = 查找边界（1）；普通节点 = 0；越界 = -1。
...
fn ffi_node_is_lookup_scope() {
    ...
    assert_eq!(
        yio_node_is_lookup_scope(h, root),
        1,
        "scene root is lookup scope"
    );
    assert_eq!(yio_node_is_lookup_scope(h, node), 0, "plain node is not");
    assert_eq!(yio_node_is_lookup_scope(h, u64::MAX), -1, "oob node -> -1");
    assert_eq!(
        yio_node_is_lookup_scope(std::ptr::null(), root),
        -1,
        "null handle -> -1"
    );
    yio_stage_free(h);
...
fn ffi_guard_swallows_panic_returns_fallback_and_counts() {
    ...
    assert_eq!(yio_ffi_panic_count(), 1, "panic counted once");
    assert_eq!(ffi_guard(9i32, || 9), 9, "happy path passes through");
    assert_eq!(yio_ffi_panic_count(), 1, "happy path not counted");
    crate::FFI_PANIC_COUNT.store(0, Ordering::Relaxed);
}
...
fn stage_tween_spec_end_to_end_with_repeat_and_bezier() {
    ...
    crate::yio_stage_tween_spec(h, root, &spec, start.as_ptr(), end.as_ptr());
    ...
    crate::yio_stage_tween_spec(h, root, &bad, start.as_ptr(), end.as_ptr());
    // null spec → no-op
    crate::yio_stage_tween_spec(h, root, std::ptr::null(), start.as_ptr(), end.as_ptr());
