...
use crate::*;
use ikat_core::render::node::ChangeLevel;
...
fn ffi_set_node_visible_emits_invisible_row() {
    ...
    let kind = b"div";
    let root = ikat_stage_create_root(h, kind.as_ptr(), kind.len(), std::ptr::null(), 0);
    assert_ne!(root, 0, "create_root");

    let css = b"position:absolute;left:0;top:0;width:200px;height:100px;background-color:#123456";
    let n1 = ikat_stage_create_node(h, kind.as_ptr(), kind.len(), css.as_ptr(), css.len());
    assert_ne!(n1, 0);
    let n2 = ikat_stage_create_node(h, kind.as_ptr(), kind.len(), css.as_ptr(), css.len());
    assert_ne!(n2, 0);
    assert_eq!(ikat_stage_append_child(h, root, n1), 0);
    assert_eq!(ikat_stage_append_child(h, root, n2), 0);
    ...
    let rc = ikat_stage_set_node_visible(h, n1, 0);
