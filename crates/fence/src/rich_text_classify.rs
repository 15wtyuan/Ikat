...
//!
//! Yio 运行时只在一种上下文里实现浏览器式 inline flow：**rich-text-block**——
...
fn span_declares_flex(
    ...
    single_compound_flex_rules: &[&yio_core::style::dynamic::Compound],
...
///   整体测量）。Yio 的 inline→flex 是 taffy 兼容 hack，但「flex 容器 + padding +
            ...
            diagnostics.push(Diagnostic::error(
                ...
                format!(
                    "<{tag}> mixes inline children (text/span/img) with block children \
                     (div/controls/template/slot). Yio rich-text inline flow requires the \
        ...
        if outside {
            diagnostics.push(Diagnostic::error(
                ...
                "<a> is only valid inside a rich-text-block context (a block \
                 ...
                 non-flex <span>). Yio folds the <a> subtree into the parent's \
...
fn collect_sizing_class_rules(
    dynamic_rules: &[yio_core::style::dynamic::DynamicRule],
) -> Vec<&yio_core::style::dynamic::Compound> {
    dynamic_rules
        .iter()
        .filter(|r| {
...
fn el_declares_sizing(
    el: &crate::ir::IrElement,
    sizing_rules: &[&yio_core::style::dynamic::Compound],
) -> bool {
