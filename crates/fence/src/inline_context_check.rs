...
//!
//! taffy 0.12 不支持 CSS inline flow（inline 元素自动横排换行）。Yio 只在一种上下文里
//! 让 inline 元素和浏览器一致：**flex 容器内**——inline 元素是 flex item，按 flex 规则排
//! （两边行为相同）。
//!
//! 在 **block 容器**里（裸 `<div>` 等），Yio 把 inline 标签当 block-level（撑满父宽 + 竖排），
...
use yio_core::style::dynamic::{AttrOp, Compound, DynamicRule};
...
pub fn check_inline_context(
    ...
    for (idx, node) in tree.nodes.iter().enumerate() {
        ...
        // （display:flex 的 inline 元素在浏览器仍 shrink-to-fit，和 Yio 撑满不一致 → 不放行。）
        ...
        diagnostics.push(Diagnostic::error(
            ...
            format!(
                "inline element <{tag}> is directly inside a block container — \
                 Yio renders it block-level (fills width + stacks vertically), \
                 which differs from the browser's inline behavior (shrink-to-fit + horizontal flow). \
                 Yio has no inline flow outside flex. \
                 Fix (pick one): \
