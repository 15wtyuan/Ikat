...
use loomgui_core::style::mapping::parse_transition;
use loomgui_core::style::resolved::ResolvedStyle;
use loomgui_core::tween::TweenProp;
...
fn specs_cover(specs: &[loomgui_core::style::resolved::TransitionSpec], prop: TweenProp) -> bool {
    specs
        .iter()
        ...
        out.push(Diagnostic::error(
            ...
            format!(
                "transition {prop_name}: endpoint {} is not animatable ({}) — use explicit \
                 ...
                 measurement, which LoomGUI does not do yet), so the fence rejects them.",
                d.label(),
                values.join(", ")
            ),
            loc,
