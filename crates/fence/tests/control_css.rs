//! 控件必须被 CSS 命中校验（端到端）。
//!
//! Yio 控件（role 驱动：progressbar/slider/switch/radio/textbox/...）不带 UA
...
fn has_control_css_diag(result: &yio_fence::pipeline::ParsedTemplate, needle: &str) -> bool {
    result
        .diagnostics
        .iter()
...
fn progressbar_without_css_message_is_actionable() {
    ...
    assert!(
        !d.message.contains(".yio-"),
        "msg 不应再引用已删除的 .yio-* 注入: {}",
        d.message
    );
}
...
// （边框/底色/光标），但 Yio core 无 UA 表——打包后运行时空白。
