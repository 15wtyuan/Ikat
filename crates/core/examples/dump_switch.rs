...
//!
//! 用法：cargo run -p yio_core --example dump_switch -- <final> [seq...] [--runs N]
//!   序列为页面名列表，按顺序 instantiate→tick→remove，最后一个保留并 dump。
//!   默认 runs=8（每次全新 Stage）。
//!
//! 例：cargo run -p yio_core --example dump_switch -- settings home settings home settings
//!     cargo run -p yio_core --example dump_switch -- inventory home mail home inventory --runs 12
//!
