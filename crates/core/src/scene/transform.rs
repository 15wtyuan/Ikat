...
fn rec(scene: &Scene, anim: &AnimTable, id: NodeId, parent_world: Affine2, worlds: &mut [Affine2]) {
    ...
    let world = match node.parent.and_then(|p| scene.scroll.get(p)) {
        Some(st) => transform::mul(
            &parent_world,
            &transform::mul(
                &transform::from_translate(-st.scroll_pos.0, -st.scroll_pos.1),
                &local,
            ),
        ),
...
mod tests {
    ...
    fn identity_nodes_world_is_translation_to_layout() {
        ...
        assert!(
            s.world_transforms[cid.index()].is_pure_translation(),
            "identity 子树 world 纯平移"
        );
    }
    ...
    fn child_inherits_parent_rotation() {
        ...
        assert_eq!(
            s.world_transforms[cid.index()],
            s.world_transforms[rid.index()],
            "identity 子继承父 world"
        );
        assert!(
            !s.world_transforms[rid.index()].is_pure_translation(),
            "父旋转 → world 非纯平移"
        );
    }
    ...
    fn skew_composite_propagates_to_child_world() {
        ...
        let cid = child_id(&s, rid, 0);
        s.get_mut(rid).unwrap().style.transform = LocalTransform {
            matrix: transform::from_scale(2.0, 1.0),
        };
        s.get_mut(cid).unwrap().style.transform = LocalTransform {
            matrix: transform::from_rotate(std::f32::consts::FRAC_PI_4),
        };
        compute_world_transforms(&mut s);
        assert!(
            !s.world_transforms[cid.index()].is_pure_translation(),
            "父非均匀缩放+子旋转 → world 剪切（非纯平移）"
        );
    }
    ...
    fn translate_stacks_on_layout_rel() {
        ...
        let cid = child_id(&s, rid, 0);
        s.get_mut(cid).unwrap().style.transform = LocalTransform {
            matrix: transform::from_translate(5.0, 0.0),
        };
        compute_world_transforms(&mut s);
        let (x, y) = s.world_transforms[cid.index()].apply_point(0.0, 0.0);
        assert!(
            (x - 15.0).abs() < 1e-4 && y.abs() < 1e-4,
            "translate 叠 layout rel 不双计：rel(10)+t(5)=15"
        );
    }
    ...
    fn anim_transform_override_replaces_css_matrix() {
        ...
        assert!(
            !s.world_transforms[rid.index()].is_pure_translation(),
            "anim.transform override 生效（scale）"
        );
    }
    ...
    fn no_anim_falls_back_to_css_matrix_zero_regression() {
        ...
        assert!(
            (x - 5.0).abs() < 1e-4 && (y - 7.0).abs() < 1e-4,
            "无 anim → CSS identity 纯平移"
        );
    ...
    fn scroll_offset_applies_to_children_not_container() {
        ...
        assert!(
            ...
            "容器自身 world 不吃自己 scroll_pos"
        );
        // 子 world.apply(0,0) = (0, -30)（含 T(-scroll_pos)）
        let (x, y) = s.world_transforms[cid.index()].apply_point(0.0, 0.0);
        assert!(
            x.abs() < 1e-3 && (y - (-30.0)).abs() < 1e-3,
            "子吃父 scroll offset：(0,30)→(0,-30)"
        );
    }
    ...
    fn scroll_pos_zero_is_no_op_zero_regression() {
        ...
        assert_eq!(
            ...
            "scroll_pos=0 no-op：与无 scroll 表项等价"
        );
    }
