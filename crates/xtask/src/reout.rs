...
//! ikat.exe + ikat_gui.exe → 重打 showcase bundle → 重打 HeadlessTests fixture 包」
//! 一条命令（AGENTS 的 Rust→dll 闭环机械化）。GUI exe 与其余产物**无条件同批重出**
//! ——它直链 ikat_pkg/ikat_fence（dev fallback 进程内路径），判据省不掉遗漏风险，
//! 直接无判据。
//!
...
const ARTIFACT_PATHS: [&str; 6] = [
    ...
    "tests/dotnet/Ikat.HeadlessTests/fixtures",
];

/// HeadlessTests fixture 根：每个 `<name>.workspace/` 的构建产物拷成
/// `<name>.pkg.bin`（静态 `fonts/` 与构建现场 `*-ws-out/` 不在此列）。
const FIXTURES_DIR: &str = "tests/dotnet/Ikat.HeadlessTests/fixtures";
...
pub fn run_reout(dry_run: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    rebuild_headless_fixtures(&root, &target.join("ikat.exe"))?;
...
/// 重打 HeadlessTests 的全部 fixture 包：每个 `<name>.workspace/` 跑 `ikat build`，
...
fn rebuild_headless_fixtures(root: &Path, exe: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for ws in &workspaces {
        let out = Command::new(exe)
            ...
            .current_dir(root)
            .output()
            .map_err(|e| format!("run ikat build {}: {e}", ws.display()))?;
        ...
        let cfg: serde_json::Value = serde_json::from_str(
            &std::fs::read_to_string(ws.join("ikat.workspace.json"))
                .map_err(|e| format!("read {}: {e}", ws.join("ikat.workspace.json").display()))?,
        )
