//! GUI exe（ikat_gui.exe，Tauri 2）构建：与 dll/ikat.exe **无条件一同重出**——
//! GUI 直链 ikat_pkg/ikat_fence（dev fallback 进程内路径），任何 pkg/fence 改动都编进
...
//! target/release/ikat_gui.exe（GUI 是 workspace 成员，target 走 CARGO_TARGET_DIR）。

use std::path::{Path, PathBuf};
use std::process::Command;

pub const GUI_EXE_NAME: &str = "ikat_gui.exe";
