...
**更新（2026-08-19 拓扑重构后失效）**：随包副本与该字节门已删——fence 内容的唯一工作区出口是 loomgui-editor skill 的 `references/css-reference.md`（fence-sync 锚点门在 loomgui_gui 的 `templates_sync.rs`）。本坑保留作「双副本必漂移」的教训记录。
