# LoomGUI 路线图（归档：摸黑打通 + 三束加宽 纪元）

> **⚠️ 本文件已归档**（2026-08-11）。记录的是「API 范式重构」纪元——从 v1「stage 全局 + NodeId 句柄 + 命令式 SetX」重做成「类型化对象树 + 标准 HTML 围栏 + CSS 优先」，经「摸黑打通骨架链（终点线1 headless + 终点线2 Unity）」到「三束加宽（控件/复合/视觉）」的全过程。**该纪元已完工**，后续规划见新 `docs/roadmap/roadmap.md`。
>
> **纪元终态（2026-08-11）**：
> - **摸黑打通 ✅**（Spec-1 → Spec-4b）：core 类型化重构（NodeKind 21 变体 + side table）+ cascade 引擎（选择器解析 + 继承 + specificity）+ 打包编排（IrTree→TemplateNode 桥）+ C# 投影层（typed 对象树 + 事件层）+ 多引擎后端分层（LoomHost/LoomBackend，LoomStage 退役）+ Unity 端到端真机验收。
> - **控件束 ✅ 近全完成**：ProgressBar/Toggle/Slider/RadioButton、TextField/PasswordField/SearchField/TextArea + IME、NumberField、Dropdown/OptionItem、TabList/Tab 全栈；C# 投影全 21 变体无 Container fallback。缺 Tree（按需）+ 一批 Unity PlayMode 验收 defer 家里机。
> - **复合束 ListView ✅**：parked-but-attached 持久 slot 池 + keepalive 子树 + notify 过滤 + L1 子树查找修 `Get<T>`（坑 182/184/185 解）。RichText 作 NodeKind 已退役（measure 原语留）。Custom Element+slot（空壳）、scope 三件套（IsScopeRoot L3/per-scope 去重/Shadow DOM）仍 defer。
> - **视觉束 ✅ 大半完成**：box-shadow 多层/inset/gaussian SDF（pkg v32）、border-radius 椭圆角、九宫格、文字特效（glow/outline/shadow/blur SDF）、transform（NodeTransform TRS + set_transform FFI）、filter 矩阵、opacity 父级累积、M2 动画（KeyframePlayer + transition + @loom-hook + :nth-child）。**缺**：渐变（仅 linear 2 色 4 方向）、`filter:blur()`、`grayed` 字段。
> - **基建 ✅**：unity-debug-bridge（DumpSceneJson + unity-cli-loop）、release CI（tag 触发 + release-check 门）、showcase 打包 exit 0（9 组件）。
>
> 权威设计契约（仍活）：`docs/design/main-design.md`（总体架构）、`docs/design/public-api.md`（公共 API 终态）、`docs/design/projection-layer.md`（C# 投影层机制）、`docs/design/fence.md`（围栏）。
...
> **状态：DONE**。spec `docs/archive/superpowers-specs-2026-08/2026-07-17-cascade-finalization-and-projection-design.md`、plan `docs/superpowers/plans/2026-07-17-cascade-finalization.md`。③ 验收（终点线1 加固）：probe HTML 经新查询出口断言 继承/specificity/class/kind 保真 全绿——cascade 引擎健全坐实，「rect 对 ≠ 语义对」盲区消除。
>
...
> **状态：DONE**。spec `docs/archive/superpowers-specs-2026-08/2026-07-16-ir-bridge-packer-design.md`、plan `docs/superpowers/plans/2026-07-16-ir-bridge-packer.md`。原三 bullets 全落地（见下"原 spec"）。终点线1 smoke 绿：HTML 字符串 → 内存 pkg.bin → `load_package` → `instantiate` → `tick_and_render` → 断言 rect（width=200）+ display:none 子树剪枝 + class 命中 computed style（`tests/smoke_ir_bridge.rs`）。
>
...
> **状态：DONE**。spec `docs/archive/superpowers-specs-2026-08/2026-07-18-spec4b-unity-acceptance-and-backend-retirement-design.md`、plan `docs/superpowers/plans/2026-07-18-spec4b-p3-unity-acceptance.md`。三 phase 落地：
>
...
> **🔔 纪元终态快照（2026-08-11，归档时核）**：下方各束的逐 P 进度笔记是历史快照（写到 2026-08-04），后续 08-06→08-11 的收口工作集中归档在 §8 末「纪元收尾」条。终态一句话：**控件束 ≈ 全完成**（21 NodeKind 全 + C# 投影无 fallback，缺 Tree 按需）；**复合束 ListView ✅**（pooled slot 池），文本模型 RichText 变体已退役，Custom Element+slot/scope 三件套仍 defer；**视觉束大半完成**（box-shadow/border-radius/九宫格/文字特效/transform/filter 矩阵/动画 M2 全交付），缺渐变补全 + filter:blur + grayed。详细缺口见 §8 末条 + tech-debt 段。
...
>
> **P3（编码端 DONE 2026-07-28，Unity PlayMode 验收 defer 家里机）= 控件债收口 + Dropdown 全栈**：spec `docs/archive/superpowers-specs-2026-08/2026-07-28-controls-debt-and-dropdown-design.md`、plan `docs/superpowers/plans/2026-07-28-controls-debt-and-dropdown.md`，29 commits（`cefbca3a..009e0b7a`）。SDD 全程实现 + per-task review + final whole-branch review。**逻辑全绿**（854 core + 361 headless + PublicApi + showcase pkg exit 0，fmt/clippy 清）；**Unity PlayMode 四门验收 defer 家里机**（页 `showcase/spec4b/dropdown-acceptance.html` + pkg + checklist 备好）。一个 spec 一批做掉——① NumberField（数值约束层，TextField 变体）；② 文本控件 getter 缺口（TextField/TextArea 的 ReadOnly/Disabled getter + Blur()，根因是 core FFI 无 `get_control_readonly`/`get_node_disabled`/`blur` 导出，数据 core 齐全）；③ 控件 C# 投影类补齐（OptionItem/Slot/CustomElement，当前 NodeFactory fallback Container）；④ UIStyleException（public-api 声明 4 种异常，Types.cs 缺此）；⑤ 攒批回写 flush（StyleMirror setter 立即过桥改帧末）+ set_transform 接通（FFI 已存在 `loomgui_stage_set_transform`，C# NodeTransform 只写镜像未调 FFI）；⑥ **Dropdown/OptionItem 全栈 + overlay 渲染通道**（见下）。这一批不碰 cascade/attr 主路径。

> **P4（M3 TabList 编码端 DONE 2026-08-04，Unity PlayMode 验收 defer 家里机）= WAI-ARIA 复合控件先行**：spec `docs/archive/superpowers-specs-2026-08/2026-08-04-m3-tablist-design.md`、plan `docs/superpowers/plans/2026-08-04-m3-tablist.md`，12 commits（`acdc658..ca3fa99`，branch `m3-tablist`，已 merge main）。SDD 全程 + per-task review + final whole-branch review「Ready to merge: Yes」。**逻辑全绿**（cargo 全 crate + dotnet 361 + PublicApi，fmt/clippy 清）。镜像 Dropdown 全套机制：`NodeKind::TabList/Tab` + `ControlState::TabList{selected_index}` + `synth_aria_value` 跨节点合成 aria-selected（Tab 无状态，从父派生）+ panel 显隐（`RoleInfo.aria_controls` + 每帧 `find_by_id_attr` + `set_inline_override` 复用 display:none 剪枝）+ click/方向键(row/col/reverse) + 复用 `EVT_SELECTION_CHANGED` + C# `TabList`/`Tab` 投影。**关键定论**：走 α 路线（框架控件，复用 ControlState/synth/role-dispatch 既有机制）不做通用 attrs 仓库 β（aria-controls 用特定 `RoleInfo` 字段，非通用 HashMap）——一致性 + AI 先验（HTML 连行为也描述全）+ β 可复用优势在 showcase 无第二用例（YAGNI）。pkg v28→v29（TemplateNode.aria_controls）。SDD 挖出坑 176（plan task 序≠编译依赖序）+ 177（pkg bump staleness）+ 178（RoleInfo::is_empty）。**Tree 推迟**（按需）。showcase settings.html `.tab[aria-selected]` 规则已解注释。
>
...
>
> **P4 = WAI-ARIA 复合控件（TabList/Tree）**。**TabList 已落地（M3，2026-08，编码端 DONE）**：spec `docs/archive/superpowers-specs-2026-08/2026-08-04-m3-tablist-design.md`、plan `docs/superpowers/plans/2026-08-04-m3-tablist.md`。原「技术线独立」的前提（① Node 不存属性 ② role 不分派 ③ attr_matches_node 硬编码）已被控件束 P1-P3 关掉后两条（见 tech-debt `[aria-selected]` 条 RESOLVED），M3 按 α 路线（特定属性 RoleInfo.aria_controls 存储，不做通用 attrs 仓库 β）落地 TabList 复合控件。**Tree 推迟**（按需）。
...
- ✅ **keyframes runtime 驱动 + fence 动画子集 — RESOLVED（功能层）**（M2 @keyframes runtime，2026-08）：M2 交付功能完整的动画系统（spec `docs/archive/superpowers-specs-2026-08/2026-08-04-m2-keyframes-runtime-design.md`，plan `docs/superpowers/plans/2026-08-04-m2-keyframes-runtime.md`，branch `m2-keyframes`）。原列缺口状态：
  - ✅ **runtime 驱动**：`KeyframesTable` 进 pkg（v30→v31）+ `ResolvedStyle.animation` 字段 + bridge 序列化 + `KeyframePlayer`（路线甲：独立 player 不翻译成 Tween 序列）+ tick step a `player.update` / step g' `sync_animation_players`。
  - ✅ **`transition` 真生效**：fence 解析存值 + core transition 引擎消费（`AnimationSpec`/`TransitionSpec` 委托 core `parse_animation`/`parse_transition` 共用同一解析器）。
  - ✅ **`animation-delay` 标准 CSS 语法**：spec §8.2 区分 duration（首个时间）/ delay（第二个时间），不再“最后一个数值 token 当 delay”。
  - ✅ **`@loom-hook`**：fence 解析 `/* @loom-hook name */` 锦点（挂 stop），player 跨越 emit `AnimationHookEvent`。
  - ⏳ **无 `animation` 长划子属性**（defer M2.5）：仅 `animation` 简写存在，缺标准 CSS 8 个长划属性。
  - ⏳ **缓动仅 7 keyword**（defer M2.5）：`linear`/`ease`/`ease-in`/`ease-out`/`ease-in-out`/`step-start`/`step-end`。无 `cubic-bezier()`/Elastic/Bounce/Custom。
  - ⏳ **keyframes 内不支持 per-stop 缓动**（defer M2.5）：标准 CSS 每个 stop 可带 `animation-timing-function`，当前不支持。
  - 上述 ⏳ 项 = 动画**引擎终态**，归 M2.5（见 milestones + 下方「动画系统终态」条从悬置 → M2.5 立项）。opacity 父级累积传播（DFS `parent_alpha × own_opacity`）顺手在 M2 交付（render/mod.rs）。
...
- ✅ **`[aria-selected]` state-attr selector + role dispatch — RESOLVED**（M3 TabList，2026-08）：原列三条串连前置缺口，两条在控件束 P1-P3 已做掉，第三条 M3 按 α 路线落地——
  ...
  - ✅ **③ role dispatch — DONE**（控件束 P1-P3，commit `a797840`）：`resolve_semantic` 先查 role，`ROLE_TO_SEMANTIC` 已含控件 role（combobox/option/listbox/slider/spinbutton/switch/radio/progressbar/list/listitem）；M3 补 `tablist`/`tab`。对照 blitz（`accessibility.rs`）实证坐实：role 默认只做 a11y 输出，仅复合控件显式读 role 决定内部行为。
  - **M3 真实落地 = TabList 复合控件**（spec `docs/archive/superpowers-specs-2026-08/2026-08-04-m3-tablist-design.md`）：镜像 Dropdown 全套机制——`SemanticKind::TabList/Tab`（fence）→ `map_semantic` 1:1 → `NodeKind::TabList/Tab`（core）+ `ControlState::TabList{selected_index}` + `synth_aria_value` 跨节点合成 aria-selected（Tab 无状态，从父 selected_index 派生）+ click/方向键移动 selected_index + panel 显隐（`sync_control_visuals` 据 `RoleInfo.aria_controls` 每帧 `find_by_id_attr` 解析 + `set_inline_override` display:block/none）+ `EVT_SELECTION_CHANGED` 复用 + C# 投影 `TabList.SelectedIndex`/`SelectionChanged`。`[aria-selected]` 运行时语义 = Tab 节点的 aria-selected 由 synth 合成（非字面存储），`attr_matches_node` 走 `[aria-*]` synth 分支查到。showcase `settings.html` `.tab[aria-selected="true"]` 规则已解注释。
...
- **Phase1 Spec-P2 视觉装饰 — 范围收窄**（2026-07-21 P2 plan 勘察后决策）：阶段1 spec §5 原列 A 圆角 + G 多层 background + I 渐变 + J 字体单位四项，实证勘察后收窄为 **只 A 圆角边框**（I-linear 在 plan 细化时进一步 defer），G/I/J 全 defer。**A 已完成 @ `303e6e7`（Task 1-2，review Approved）；Task 3-5（linear）defer**：
  - ✅ **A 圆角边框 — RESOLVED**（Phase1 Spec-P2 @ `303e6e7`）：`border_ring` 消费 radii 画圆角环（外/内轮廓配对三角带 + 内外同分段 infill 扇）+ `box_shadow_quad` 圆角，删两处 `let _ = radii`；`mesh.rs` 抽 `radius_scale` + `corner_arc_pts` 公共 helper（`rounded_rect` DRY 共用，保背景圆角==边框圆角）。dll 重编入库；几何 review Approved（opus）。**headless 端到端验收补全**（`showcase/spec4b/p2-visual-acceptance.html` #rb border+radius / #rs shadow+radius + `tests/dotnet/LoomGUI.HeadlessTests/VisualDecorationTests.cs`：border 声明进 computed style `border_present=1` + red + width 到 layout，304 HeadlessTests 绿；圆角顶点几何仍归 `render/tests.rs`、视觉对齐归 Unity PlayMode）。Task 3-5（linear 角度）仍 defer。
  - **I linear 扩展**（defer §4 视觉束）：plan 细化时实证发现——多 stop 需细分 mesh（4 角顶点色双线性是 affine 色场，表达不了 stop 分段：`red,yellow,blue` 沿 to right 中间应 yellow、双线性给紫色），需沿渐变线切 N 段 sub-quad；任意角度（45deg）2 色 per-vertex 可行，但 `Gradient2` 加 `angle` 字段需 pkg bump（21→22）只为 45deg 2 色不划算；corner 方向（`to top right`）按 rect 长宽比算角度复杂。**整项 defer 到真需要多 stop 时合并做**（角度+多 stop+corner 一次 bump）。P2 本轮只做 A。
  - **G 多层 background**（defer §4 视觉束）：LoomGUI 完全单层，多层=大重构（`resolved`+`Vec<BackgroundLayer>`/`mapping` 逗号 split 带括号深度/`render` 多层循环/`fence` schema 扩 + 实现死枚举 `BackgroundShorthand`/`pipeline` sprite 多 URL），且有陷阱：围栏对 `url(a),url(b)` 经 `parse_url` 静默损坏（得 `a),url(b`）。游戏 UI 99% 单层 bg，ROI 低。blitz 多层靠 stylo 周期扩展（`get_cyclic`），LoomGUI 手搓 CSS 得自己实现。
  - **I radial/conic**（defer §4 视觉束）：per-vertex 做不了（`|p-c|`/`atan2` 非 affine，双线性插值出 affine 色场），要新 Unity shader program=5 + 1D LUT 纹理上传 + Unity 后端同步，编码机验不了 shader（家里机 PlayMode 串行验），工作量大；v1.8 spec line 60 明文 defer（"per-vertex 做不了要 shader"）。showcase `home.html` radial 装饰光晕当前静默丢弃。真有圆环 cooldown/光晕需求时做。
  - **J ex/ch/ic**（defer YAGNI）：`parse_lp` 连 `em`/`rem` 都没支持，补 ex/ch/ic 错位；游戏 UI 不用；showcase 零使用。代价高（`parse_lp` 签名改 + 所有调用点连锁 + `apply_decl` 取 FontTable + ex fallback `x_height==0→0.5em`）。未来有 CJK 排版需求再补（且应先补 em/rem 更基础）。
...
- **动画系统终态**（M2.5 立项，不再悬置）：M2 交付功能完整的动画系统（KeyframePlayer 路线甲 + transition + Animation 句柄 + @loom-hook + :nth-child + opacity 父级累积），但引擎内部用「够 keyframes 跑」的实现——`TweenManager` 仍是单个 `Vec<Tween>`、flat `tween()` API、7 keyword 缓动、value_size max=4。终态 = 池化 `{active,pool}` + 链式 builder API + 缓动全集（cubic-bezier/Elastic/Bounce/Custom + per-stop timing-function）+ value_size(1..6) + prop_type 分层（transform_dirty 不 solve vs layout_dirty 触发 solve 重入）+ player 与 Tween 插值原语统一。归 **M2.5**（进入判据：第一个需 layout 动画的 showcase 页 / M5 NodeTransform 升级合并 / 动画并发使单 Vec 抖动）。详见 milestones M2.5 + main-design §13.6。
...
| 功能 | 旧编号 | 归属 |
|---|---|---|
| 动画 runtime 驱动（@keyframes 表 + player/ease/iteration） | v1.10 | ✅ M2 已交付（功能层完整；引擎终态 → M2.5） |
...
- **M3 TabList 复合控件 complete（2026-08-04，控件束 P4 先行）**（spec/plan 2026-08-04）：WAI-ARIA `role=tablist/tab/tabpanel` 做成框架自带复合控件，镜像 Dropdown 全套机制。关键定论：① **α vs β 决策**——框架控件（α，扩 ControlState + synth_aria_value + role 分派既有机制）优于通用 Node attrs 仓库（β）；理由 = 一致性（“其他控件都这么干”）+ AI 先验（HTML 连行为也描述全，最大化「AI 写 DSL 即懂界面」核心论点）+ β 的可复用优势在 showcase 无第二用例（YAGNI，等 Tree/Accordion 出现再补）。② aria-selected **跨节点合成**（synth 第一处跨节点：Tab 无 ControlState，向上找父 TabList.selected_index 派生——镜像 OptionItem 从父 Dropdown 派生）。③ panel 显隐 **复用 display:none 剪枝**（`set_inline_override` 切 display，不造 hidden 标志字段）——镜像 Dropdown 切 listbox display。④ panel 跨树关联靠 `aria-controls`↔id（panel 非 tablist 子节点），用 `RoleInfo.aria_controls` 特定字段存（非通用 attrs），`sync_control_visuals` 每帧 `find_by_id_attr` 解析。⑤ 复用 `EVT_SELECTION_CHANGED`（零新事件 struct/demux arm）。pkg v28→v29（TemplateNode.aria_controls）。SDD 执行挖出坑 176（plan task 序≠编译依赖序，重排 T1→T3→T4→T2→…）+ 177（pkg bump+dll 重编 staleness 在 consumer-test task 才暴露）+ 178（RoleInfo::is_empty 加字段漏更新）。spec/plan 见 `docs/superpowers/{specs,plans}/2026-08-04-m3-tablist*`，branch `m3-tablist` 已 merge main（`acdc658..ca3fa99`）。Unity PlayMode 真机验收 defer 家里机。
- **M2 @keyframes runtime + transition complete（2026-08，动画功能层全交付）**（spec/plan 2026-08-04）：M2 = home 入场动画那一秒，把 fence `@keyframes`/`animation` DSL（提前进 P3.4c）的 runtime 驱动补齐。SDD 全程（spec `docs/archive/superpowers-specs-2026-08/2026-08-04-m2-keyframes-runtime-design.md`、plan `docs/superpowers/plans/2026-08-04-m2-keyframes-runtime.md`，15 tasks + per-task review + final whole-branch review）。关键定论：① **路线甲：独立 KeyframePlayer**（不翻译成 Tween 序列）——`@keyframes` 表 + `AnimationSpec` 驱动 slotmap 稳定 `KeyframePlayer`，player 推进时间轴直接写 `NodeAnim` 四通道（transform/opacity/bg_color/text_color）。理由：keyframes 表驱动（迭代/direction/fill/per-segment ease）比拆 Tween 序列干净，transition 仍走 Tween 引擎（两条写入者 step a 并列，写入顺序即优先级——animation 后写覆盖，天然 animation 优先于 transition）。② **只动渲染层**（A1）：player/tween 只动 transform/opacity/bg_color/text_color，不动布局（layout 动画 = prop_type 分层 + tick 时序重构，defer M2.5）。③ **tick 时序加两处**：step a `KeyframePlayer.update(dt)`（与 TweenManager 并列，单一时钟语义）+ step g' `sync_animation_players`（rematch 后、solve 前，检测 computed animation 声明变化启停 class 触发的 player）。④ **Animation 句柄 L3 全套不 defer**：`Node.Play(name)` 返句柄 + 事件 START/END/ITERATION/KEY/HOOK + TransitionEnd 双路由（全局 EventBus `On<T>` + 句柄 `player_key` 私有回调）。⑤ **ITERATION CSS 语义**：最后一次 iteration 完成帧只发 END 不发 ITERATION（对齐浏览器 `animationiteration`）。⑥ **opacity 父级累积顺手做掉**（DFS `parent_alpha × own_opacity`，与浏览器 opacity 合成语义一致）。⑦ **包级 keyframes + Scene 级查找 + transform TRS 分解**（非合并矩阵）。⑧ **fence 解析委托 core**：`parse_animation`/`parse_transition` 留 core 唯一解析器，fence 调用同一函数（防 spec §8.2/§8.3 语义漂移，inline 打包期与运行时 rematch 共用）。⑨ **:nth-child(An+B|odd|even|N) + @loom-hook 锦点** 进 fence（home 导航卡错峰入场 + 命名键回调）。pkg v30→v31（`KeyframesRule`/`AnimationSpec`/player slotmap + ComponentTemplate.keyframes）。**逻辑全绿**（cargo 全 crate + dotnet HeadlessTests + PublicApi + fence doc_schema_sync，fmt/clippy 清）；Unity PlayMode 真机验收 defer 家里机（M2 验收页 `showcase/m2-animation.html` 12 块 + home.html stagger 解注释 + pkg + checklist 备好）。SDD 挖出坑（ITERATION 完成帧 bug / multi-animation 一帧闪烁 update_all clear mask = own ∩ ¬other-active / Animation registry leak on node dispose）。**动画引擎终态**（池化 Tween + 缓动全集 + 链式 builder + layout 动画 prop_type 分层）从「悬置」→ **M2.5 明确立项**（进入判据见 milestones M2.5）。spec/plan 见 `docs/superpowers/{specs,plans}/2026-08-04-m2-keyframes-runtime*`，branch `m2-keyframes`。
- **纪元收尾（2026-08-06 → 2026-08-11，三束加宽收口 + 基建）**：本纪元最后一批工作，roadmap 正文未来得及逐条记录，此处集中归档：
  - **M1 ListView 虚拟化收口 ✅**（commit `e4d40515` 等 2026-08-06）：pooled-slot-lifecycle 设计落地——parked-but-attached（slot 永驻 ul 子树，离场 `display:none` 标记而非 detach）+ 稳定 ordinal reuse_key + FFI blob parked keepalive（visible byte bit1）+ Unity MirrorPool parked 分支（reactivate/lazy）+ L1 `find_node_by_id_in_subtree`（self-exclusive DFS）修 `Get<T>` 命中 slot 内部 id。坑 182（slot GO churn）解。后续 `compute_visible_range` 计入 flex gap（坑 185，mail 视口顶空白）+ measure_text 跨帧 memoize（97.5% 命中，solve 11ms→2.4ms，坑 186 缓解）+ parked keepalive 扩 slot 子树（坑 184）。spec/plan `2026-08-05-pooled-slot-lifecycle*`。
  - **视觉束大半交付 ✅**（2026-08-09，pkg v32）：box-shadow 多层 + inset + gaussian SDF blur（paren-aware tokenizer + 多层 synth RenderNode + sort_key 分层 + Unity program=5 SHADOW_BLUR + erfc 真高斯）；border-radius 椭圆角（`CornerRadius{h,v}` + `radius_scale`/`corner_arc_pts` helper）；九宫格（`nine_slice` + `nine_slice_rounded`）；文字特效（`FontEffect` Shadow/Stroke/Glow/Blur → SDF EffectBlock）；transform（`NodeTransform` TRS 分解 + `set_transform` FFI 接通 C#，坑 174 origin / 192 双解析路径解）；filter 矩阵（grayscale/brightness/contrast/saturate/hue-rotate/invert/sepia，program 3/4）；opacity 父级累积（DFS parent_alpha × own）。spec/plan `2026-08-09-box-shadow-multilayer-inset-blur*`。**视觉束仍缺**：渐变（仅 linear 2 色 4 方向，缺多 stop / 任意角度 / radial / conic）、`filter:blur()`、`grayed` 字段（暂用 `filter:grayscale(1)`）。
  - **基建 ✅**：unity-debug-bridge（`DumpSceneJson`/`DumpMirrorPoolState` runtime accessor + unity-cli-loop + LoomBridge + .agents/.claude skills，2026-08-08）；release CI（tag 触发 `release.yml` + `xtask release-check` 门 + package metadata + CHANGELOG，2026-08-09）；坑 195（dll 字节非确定性→release-check 不用字节比较）/ 196（UPM 漏 .meta）/ 197（Unity Linear 半透明偏亮，showcase 改 Gamma）/ 200（color space 改须重启）沉淀。
  - **showcase 打包 exit 0**：9 组件（home/settings/mail/inventory/shop/character/form/lab + m2-animation）打包产 showcase.pkg.bin（~1.09MB），围栏全通过。
  - **defer 家里机的 Unity PlayMode 真机验收**：M1/M2/M3/P3 的四门验收页 + pkg + checklist 备好，待家里机集中跑。
