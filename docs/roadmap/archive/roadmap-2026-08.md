...
- **架构收口批（2026-08-17 全库架构审查产出，插队执行）**：审查结论 = 方向与里程碑排序不变；债的共性是「约束靠注释不靠测试」。五件事按优先级：
  ...
  3. ~~**C# 侧布局断言 + golden blob 对拍**~~ ✅ done（2026-08-17）：Rust 产器（`crates/ffi/src/golden_tests.rs` 固定场景，env 触发落盘 + 进程内确定性自检 + 入库文件版本货币性校验）→ golden 帧字节入库 `tests/dotnet/golden/`；C# 两侧消费——Tests.Core `GoldenFrameBlobTests`（23 列逐列值域/一致性 + 场景特征列全覆盖 + clip 表 52B entry 解码）+ HeadlessTests `GoldenEventsAndAbiLayoutTests`（EventRecord 20B 位布局 golden 对拍 + 手补镜像 PointerEvent/KeyEvent/WheelEvent/RawEventRecord 尺寸锁）。取舍：Rust 侧不做跨平台字节等值门（渐变/圆角走 libm sin/cos 有 ulp 差），货币性靠版本耦合（bump 忘再生成 → `IsValid` 红）。
