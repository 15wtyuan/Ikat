...
- **Phase1 Spec-P2 视觉装饰 — 范围收窄**（2026-07-21 P2 plan 勘察后决策）：阶段1 spec §5 原列 A 圆角 + G 多层 background + I 渐变 + J 字体单位四项，实证勘察后收窄为 **只 A 圆角边框**（I-linear 在 plan 细化时进一步 defer），G/I/J 全 defer。**A 已完成 @ `303e6e7`（Task 1-2，review Approved）；Task 3-5（linear）defer**：
  - ✅ **A 圆角边框 — RESOLVED**（Phase1 Spec-P2 @ `303e6e7`）：`border_ring` 消费 radii 画圆角环（外/内轮廓配对三角带 + 内外同分段 infill 扇）+ `box_shadow_quad` 圆角，删两处 `let _ = radii`；`mesh.rs` 抽 `radius_scale` + `corner_arc_pts` 公共 helper（`rounded_rect` DRY 共用，保背景圆角==边框圆角）。dll 重编入库；几何 review Approved（opus）。Task 3-5（linear 角度）仍 defer。
