...
- **Phase1 Spec-P2 视觉装饰 — 范围收窄**（2026-07-21 P2 plan 勘察后决策）：阶段1 spec §5 原列 A 圆角 + G 多层 background + I 渐变 + J 字体单位四项，实证勘察后收窄为 **只 A 圆角边框**（I-linear 在 plan 细化时进一步 defer），G/I/J 全 defer：
  - **A 圆角边框**（做）：`border_ring`（`render/border.rs:42` `let _ = radii`）消费 radii 画圆角环——背景圆角 v1.2 已完备（`mesh::rounded_rect`），边框直角是残留 bug（背景圆角+边框直角=边角突出），含 `box_shadow_quad:105` 同源退化。
