// Ikat Frozen Public API: Node hierarchy & controls
...
namespace Ikat
{
    ...
    public abstract unsafe class Node
    {
        ...
        public string Id
        {
            get
            {
                ...
                return TextControlFFI.ReadText(h, _id,
                    (hp, buf, cap, len) => Native.ikat_stage_get_node_id_attr(hp, _id, buf, cap, len));
            }
        }
        ...
        public Container Parent
        {
            get
            {
                ThrowIfDisposed();
                StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
                ulong parentId = Native.ikat_node_parent(h, _id);
        ...
        public NodeComputedStyle Computed
        {
            get
            {
                ...
                int rc = Native.ikat_stage_get_node_computed_style(h, _id, &repr);
                if (rc != 0)
        ...
        public void RemoveFromParent()
        {
            ...
            Native.ikat_stage_remove_child(h, parentId, _id);
        }
        ...
        public void Dispose()
        {
            ...
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            Native.ikat_stage_remove_node(h, _id);
        ...
        public bool TryGet<T>(string id, out T node) where T : Node
        {
            ...
            ulong candidate;
            fixed (byte* p = idb)
                candidate = Native.ikat_stage_find_node_by_id_in_subtree(h, _id, p, (nuint)idb.Length);
        ...
        public IReadOnlyList<Node> Query(string selector)
        {
            ...
            // DOM querySelectorAll("") 抛 SyntaxError；Ikat 容错返空（不抛——宽松查询路径）。
        ...
        public AnimationHandle Play(string name)
        {
            ...
            fixed (byte* np = nb)
            {
                ulong key = Native.ikat_stage_play_animation(h, _id, np, (nuint)nb.Length);
                if (key == 0)
                    throw new UIContractException(
        ...
        public AnimationHandle Play(string name, float durationSeconds)
        {
            ...
            fixed (byte* np = nb)
            {
                ulong key = Native.ikat_stage_play_animation_dur(h, _id, np, (nuint)nb.Length, durationSeconds);
                if (key == 0)
                    throw new UIContractException(
                        $"Play(\"{name}\"): no @keyframes with this name (keyframes table lookup failed)");
                var anim = new AnimationHandle(this, key, name);
        ...
        public void Focus()
        {
            ThrowIfDisposed();
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            Native.ikat_stage_request_focus(h, _id);
        }
        // Blur 清除当前焦点（stage::blur）：记 pending_focus_request = Some(None)，下 tick 消费清焦点
        // （与 request_focus 对称的 stage 级操作）。FFI ikat_stage_blur 不带 node_id——它清的是
        // 「当前获焦节点」而非「本节点」，故无焦点时调为 no-op（业务侧通常对聚焦控件调 Blur）。
        public void Blur()
        {
            ThrowIfDisposed();
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            Native.ikat_stage_blur(h);
        }
        ...
        public void SetPointerCapture(int touchId)
        {
            ThrowIfDisposed();
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            Native.ikat_stage_add_touch_monitor(h, touchId, _id);
        }
        ...
        public void CancelClick(int touchId)
        {
            ThrowIfDisposed();
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            Native.ikat_stage_cancel_click(h, touchId);
        }
        ...
        /// 用 ikat_node_parent 逐层向上，直到撞 _id（在子树）或 RootSentinel（走出根）。
        ...
        {
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            return Native.ikat_node_is_lookup_scope(h, _id) == 1;
        }
        ...
        private static bool MatchesTag(Node n, string tag)
        {
            ...
            byte kind = 0xFF;
            int rc = Native.ikat_stage_get_node_kind(h, n._id, &kind);
        ...
        internal void DisposeDescendantsInRegistry(ulong subtreeRootId)
        {
            ...
            int count = Native.ikat_stage_get_child_count(h, subtreeRootId);
            if (count <= 0) return;
            ulong[] buf = new ulong[count];
            int written;
            fixed (ulong* bp = buf)
            {
                written = Native.ikat_stage_get_children(h, subtreeRootId, bp, (nuint)buf.Length);
            }
        ...
        {
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            Native.ikat_stage_set_node_touchable(h, _id, v);
        }
        bool GetNodeTouchable()
        {
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            byte b = 0;
            int rc = Native.ikat_stage_get_node_touchable(h, _id, &b);
        ...
        {
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            Native.ikat_stage_set_node_focusable(h, _id, v);
        }
    ...
    public readonly unsafe struct NodeComputedStyle
    {
        ...
        readonly IkatColor _color;
        readonly IkatColor? _background;
        readonly float _opacity;
        readonly IkatColor? _border;
        ...
        internal NodeComputedStyle(ref ComputedNodeStyleRepr r)
        {
            ...
            _color = new IkatColor(r.color[0], r.color[1], r.color[2], r.color[3]);
            _background = r.bg_present != 0
                ? (IkatColor?)new IkatColor(r.background_color[0], r.background_color[1], r.background_color[2], r.background_color[3])
                : null;
            _opacity = r.opacity;
            _border = r.border_present != 0
                ? (IkatColor?)new IkatColor(r.border_color[0], r.border_color[1], r.border_color[2], r.border_color[3])
                : null;
        ...
        public IkatColor Color => _color;
        /// <summary>背景色；cascade 无 background-color 声明时 null。</summary>
        public IkatColor? Background => _background;
        public float Opacity => _opacity;
        /// <summary>边框色；无 border 声明时 null。</summary>
        public IkatColor? Border => _border;
    ...
    public sealed class NodeStyle
    {
        ...
        public IkatColor BackgroundColor
        {
            get => _mirror.Get<IkatColor>("background-color") ?? IkatColor.Unset;
        ...
        public IkatColor TextColor
        {
            get => _mirror.Get<IkatColor>("color") ?? IkatColor.Unset;
    ...
    // （IkatHost.Step flush seam / UIContext.FlushPendingWrites）调 FlushTransform 一次性送
    ...
    public sealed unsafe class NodeTransform
    {
        ...
        internal IkatVector2 _position = IkatVector2.Zero;
        internal IkatVector2 _scale = IkatVector2.One;
        internal float _rotation;
        internal IkatVector2 _origin = IkatVector2.Zero;
        ...
        public IkatVector2 Position { get => _position; set => Store(ref _position, value); }
        /// <summary>缩放（local 基）。default = One（不缩放）；setter 存镜像 + 标脏。</summary>
        public IkatVector2 Scale { get => _scale; set => Store(ref _scale, value); }
        ...
        public IkatVector2 Origin { get => _origin; set => Store(ref _origin, value); }
        ...
        internal void FlushTransform()
        {
            ...
            Native.ikat_stage_set_transform(
                h, _owner._id,
    ...
    public readonly unsafe struct NodeGeometry
    {
        ...
        /// <summary>
        /// 节点 layout 产物（solve 输出，左上 + w/h）。直读 get_node_layout_rect FFI（x/y/w/h → IkatRect）。
        /// 滞后一帧：本帧 Style 写入下帧才反映。
        /// </summary>
        public IkatRect LayoutRect
        {
            get
            {
                StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
                float x = 0, y = 0, w = 0, hh = 0;
                Native.ikat_stage_get_node_layout_rect(h, _id, &x, &y, &w, &hh);
                return new IkatRect(x, y, w, hh);
            }
        }
        ...
        /// </summary>
        public IkatRect WorldRect
        {
            get
            {
                IkatRect lr = LayoutRect;
                return LocalToGlobal(new IkatRect(0, 0, lr.Width, lr.Height));
            }
        }
        ...
        /// </summary>
        public IkatVector2 LocalToGlobal(IkatVector2 p)
        {
            GetWorldMatrix(out float a, out float b, out float c, out float d, out float tx, out float ty);
            return new IkatVector2(a * p.X + c * p.Y + tx, b * p.X + d * p.Y + ty);
        }
        ...
        public IkatRect GlobalToLocal(IkatRect r)
        {
            GetWorldMatrix(out float a, out float b, out float c, out float d, out float tx, out float ty);
            InverseAffine(a, b, c, d, tx, ty,
                          out float ia, out float ib, out float ic, out float id, out float itx, out float ity);
        ...
        void GetWorldMatrix(out float a, out float b, out float c, out float d, out float tx, out float ty)
        {
            ...
            Native.ikat_stage_get_node_world_matrix(h, _id, &la, &lb, &lc, &ld, &ltx, &lty);
        ...
        static IkatRect TransformAABB(float a, float b, float c, float d, float tx, float ty, IkatRect r)
        {
            ...
            return new IkatRect(minX, minY, maxX - minX, maxY - minY);
        }
    ...
    public unsafe class Container : Node
    {
        ...
        public int ChildCount
        {
            get
            {
                ThrowIfDisposed();
                StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
                int c = Native.ikat_stage_get_child_count(h, _id);
        ...
        public string TextContent
        {
            ...
            set
            {
                ...
                ulong textId;
                fixed (byte* tp = tag)
                    textId = Native.ikat_stage_create_node(h, tp, (nuint)tag.Length, null, 0);
                ...
                fixed (byte* tp = tb)
                {
                    int src = Native.ikat_stage_set_text(h, textId, tp, (nuint)tb.Length);
                    if (src != 0)
                        throw new InvalidOperationException("set_text on fresh TextNode failed (non-TextNode kind)");
                }

                int arc = Native.ikat_stage_append_child(h, _id, textId);
                if (arc != 0)
        ...
        public T AddChild<T>(T c) where T : Node
        {
            ...
            c.ThrowIfDisposed();
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            int rc = Native.ikat_stage_append_child(h, _id, c._id);
        ...
        public T InsertChild<T>(T c, int i) where T : Node
        {
            ...
            int rc = Native.ikat_stage_insert_before(h, _id, c._id, refId);
            if (rc != 0)
                throw new InvalidOperationException(
        ...
        public void RemoveChild(Node c)
        {
            ...
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            int rc = Native.ikat_stage_remove_child(h, _id, c._id);
            if (rc != 0)
                throw new InvalidOperationException(
        ...
        public void SwapChildren(Node a, Node b)
        {
            ...
            Native.ikat_stage_remove_child(h, _id, upperChild._id);
            Native.ikat_stage_remove_child(h, _id, lowerChild._id);
        ...
        public void RestartAnimations()
        {
            ThrowIfDisposed();
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            Native.ikat_stage_restart_animations(h, _id);
        }
        ...
        public IkatVector2 ScrollPos
        {
            get
            {
                ...
                StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
                float x = 0f, y = 0f;
                Native.ikat_stage_get_scroll_pos(h, _id, &x, &y);
                return new IkatVector2(x, y);
            }
        }

        public void ScrollTo(IkatVector2 p, ScrollBehavior b = ScrollBehavior.Smooth)
        {
            ThrowIfDisposed();
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            Native.ikat_stage_set_scroll_pos(h, _id, p.X, p.Y, (byte)(b == ScrollBehavior.Smooth ? 1 : 0));
        }
        ...
        public UITemplate GetTemplate(string name)
        {
            ...
            byte kind = 0;
            Native.ikat_stage_get_node_kind(h, candidate, &kind);
            ...
            int count = Native.ikat_stage_get_child_count(h, candidate);
            ulong[] children = new ulong[Math.Max(count, 0)];
            ulong child = RootSentinel;
            fixed (ulong* cp = children)
                Native.ikat_stage_get_children(h, candidate, cp, (nuint)children.Length);
            foreach (ulong c in children)
            {
                byte k = 0;
                Native.ikat_stage_get_node_kind(h, c, &k);
        ...
        private List<Node> MaterializeChildren()
        {
            ...
            int count = Native.ikat_stage_get_child_count(h, _id);
            ...
            fixed (ulong* bp = buf)
            {
                written = Native.ikat_stage_get_children(h, _id, bp, (nuint)buf.Length);
            }
        ...
        private void ClearDirectChildrenFFI(StageHandle* h)
        {
            ...
            for (int i = 0; i < written; i++)
            {
                ...
                Native.ikat_stage_remove_node(h, cid);
        ...
        public string Text
        {
            ...
            set
            {
                ...
                fixed (byte* p = b)
                {
                    ...
                    _ = Native.ikat_stage_set_text(h, _id, p, (nuint)b.Length);
                }
            }
        }
    }
    public unsafe class Image : Node
    {
        ...
        public string Src
        {
            ...
            set
            {
                ...
                fixed (byte* p = b)
                {
                    _ = Native.ikat_stage_set_src(h, _id, p, (nuint)b.Length);
                }
            }
        }
    }
        ...
        public string Href
        {
            get
            {
                ...
                string v = TextControlFFI.ReadTextOrNull(h, _id,
                    (hp, buf, cap, len) => Native.ikat_stage_get_link_href(hp, _id, buf, cap, len));
                if (v == null)
                    throw new UIContractException(
    ...
    public unsafe class Button : Container
    {
        ...
        void SetNodeDisabled(bool v) { StageHandle* h = (StageHandle*)_ctx._stage.ToPointer(); Native.ikat_stage_set_node_disabled(h, _id, v); }
        bool GetNodeDisabled() { StageHandle* h = (StageHandle*)_ctx._stage.ToPointer(); byte b = 0; Native.ikat_stage_get_node_disabled(h, _id, &b); return b != 0; }
    ...
    internal static unsafe class TextControlFFI
    {
        ...
        internal static string GetControlText(StageHandle* h, ulong id) =>
            ReadText(h, id, (hp, buf, cap, len) => Native.ikat_stage_get_control_text(hp, id, buf, cap, len));
        ...
        internal static void SetControlText(StageHandle* h, ulong id, string v)
        {
            ...
            fixed (byte* bp = b)
            {
                int rc = Native.ikat_stage_set_control_text(h, id, bp, (nuint)b.Length);
        ...
        internal static string GetControlPlaceholder(StageHandle* h, ulong id) =>
            ReadText(h, id, (hp, buf, cap, len) => Native.ikat_stage_get_control_placeholder(hp, id, buf, cap, len));
        ...
        internal static void SetControlPlaceholder(StageHandle* h, ulong id, string v)
        {
            ...
            fixed (byte* bp = b)
            {
                int rc = Native.ikat_stage_set_control_placeholder(h, id, bp, (nuint)b.Length);
        ...
        {
            nuint start = 0, end = 0;
            int rc = Native.ikat_stage_get_selection(h, id, &start, &end);
        ...
        {
            int rc = Native.ikat_stage_set_selection(h, id, (nuint)anchor, (nuint)cursor);
        ...
        {
            int rc = Native.ikat_stage_set_control_readonly(h, id, v);
        ...
        {
            byte b = 0;
            int rc = Native.ikat_stage_get_control_readonly(h, id, &b);
        ...
        {
            Native.ikat_stage_set_node_disabled(h, id, v);
        }
        ...
        internal static void SetControlMaxLength(StageHandle* h, ulong id, int v)
        {
            ...
            int rc = Native.ikat_stage_set_control_maxlength(h, id, (nuint)v);
        ...
        {
            nuint v = 0;
            int rc = Native.ikat_stage_get_control_maxlength(h, id, &v);
        ...
        {
            byte b = 0;
            Native.ikat_stage_get_node_disabled(h, id, &b);
            return b != 0;
        }
    ...
    public unsafe class NumberField : Node
    {
        ...
        float GetNumberValue()
        {
            ...
            float v = 0f; int rc = Native.ikat_stage_get_number_value(h, _id, &v);
        ...
        {
            StageHandle* h = Handle();
            int rc = Native.ikat_stage_set_number_value(h, _id, v);
        ...
        {
            StageHandle* h = Handle();
            float v = 0f; int rc = Native.ikat_stage_get_control_min(h, _id, &v);
        ...
        {
            StageHandle* h = Handle();
            float v = 0f; int rc = Native.ikat_stage_get_control_max(h, _id, &v);
        ...
        {
            StageHandle* h = Handle();
            float v = 0f; int rc = Native.ikat_stage_get_control_step(h, _id, &v);
        ...
        {
            StageHandle* h = Handle();
            int rc = Native.ikat_stage_set_control_step(h, _id, v);
    ...
    public unsafe class Slider : Node
    {
        ...
        {
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            float v = 0f; int rc = Native.ikat_stage_get_control_value(h, _id, &v);
        ...
        {
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            int rc = Native.ikat_stage_set_control_value(h, _id, v);
        ...
        {
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            Native.ikat_stage_set_node_disabled(h, _id, v);
        }
        bool GetNodeDisabled()
        {
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            byte b = 0;
            Native.ikat_stage_get_node_disabled(h, _id, &b);
            return b != 0;
        }
    }
    ...
    public unsafe class Toggle : Node
    {
        ...
        {
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            bool v = false; int rc = Native.ikat_stage_get_control_checked(h, _id, &v);
        ...
        {
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            int rc = Native.ikat_stage_set_control_checked(h, _id, v);
    ...
    public unsafe class RadioButton : Node
    {
        ...
        public string Name
        {
            get
            {
                ...
                StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
                return TextControlFFI.ReadText(h, _id,
                    (hp, buf, cap, len) => Native.ikat_stage_get_radio_name(hp, _id, buf, cap, len));
            }
        }
    ...
    public unsafe class Dropdown : Node
    {
        ...
        public string SelectedValue
        {
            get
            {
                ...
                return TextControlFFI.ReadTextOrNull(h, _id,
                    (hp, buf, cap, len) => Native.ikat_stage_get_dropdown_selected_value(hp, _id, buf, cap, len));
            }
        }
        ...
        {
            StageHandle* h = Handle();
            uint v = 0; int rc = Native.ikat_stage_get_dropdown_selected_index(h, _id, &v);
        ...
        {
            StageHandle* h = Handle();
            int rc = Native.ikat_stage_set_dropdown_selected_index(h, _id, v);
    ...
    public unsafe class OptionItem : Container
    {
        ...
        public string Value
        {
            get
            {
                ...
                StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
                return TextControlFFI.ReadText(h, _id,
                    (hp, buf, cap, len) => Native.ikat_stage_get_option_value(hp, _id, buf, cap, len));
            }
        }
        ...
        public bool Selected
        {
            get
            {
                ThrowIfDisposed();
                StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
                int rc = Native.ikat_stage_is_option_selected(h, _id);
                if (rc < 0)
        ...
        public int Index
        {
            get
            {
                ThrowIfDisposed();
                StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
                int idx = Native.ikat_stage_get_option_index(h, _id);
                if (idx < 0)
    ...
    public unsafe class TabList : Container
    {
        ...
        {
            StageHandle* h = Handle();
            uint v = 0; int rc = Native.ikat_stage_get_tablist_selected_index(h, _id, &v);
    ...
    public unsafe class Tab : Container
    {
        ...
        public bool Selected
        {
            get
            {
                ThrowIfDisposed();
                StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
                int rc = Native.ikat_stage_is_tab_selected(h, _id);
                if (rc < 0)
    ...
    public unsafe class ProgressBar : Node
    {
        ...
        bool GetControlIndeterminate()
        {
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            byte b = 0;
            int rc = Native.ikat_stage_get_control_indeterminate(h, _id, &b);
        ...
        {
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            int rc = Native.ikat_stage_set_control_indeterminate(h, _id, v ? (byte)1 : (byte)0);
        ...
        internal float GetControlValue()
        {
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            float v = 0f;
            int rc = Native.ikat_stage_get_control_value(h, _id, &v);
            if (rc != 0) throw new InvalidOperationException($"get_control_value failed (node {_id})");
            return v;
        }
        void SetControlValue(float v)
        ...
        float GetControlMax()
        {
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            float v = 0f;
            int rc = Native.ikat_stage_get_control_max(h, _id, &v);
            if (rc != 0) throw new InvalidOperationException($"get_control_max failed (node {_id})");
            return v;
        }
        void SetControlMax(float v)
    ...
    public unsafe class ListView : Container
    {
        ...
        /// <summary>
        /// 项数（数据驱动）。setter 调 ikat_list_set_item_count：首次调用若该 ul 尚未进入
        ...
        public int ItemCount
        {
            ...
            set
            {
                ...
                if (!_firstItemCountSet)
                {
                    ...
                    Native.ikat_list_drain_now(h, _id);
        ...
        public void ScrollToItem(int i, ScrollBehavior b = ScrollBehavior.Smooth)
        {
            ...
            int rc = Native.ikat_list_scroll_to(h, _id, i, behavior);
            if (rc != 0)
                throw new UIContractException(
        ...
        public void RefreshItem(int i)
        {
            ...
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            int rc = Native.ikat_list_refresh(h, _id, i, 1);
            if (rc != 0)
                throw new UIContractException(
        ...
        public void RefreshItems()
        {
            ThrowIfDisposed();
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            int rc = Native.ikat_list_refresh(h, _id, 0, _itemCount);
            if (rc != 0)
                throw new UIContractException(
        ...
        public void NotifyInserted(int i, int c = 1)
        {
            ...
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            int rc = Native.ikat_list_notify(h, _id, (byte)0, i, c);
            if (rc != 0)
                throw new UIContractException(
        ...
        public void NotifyRemoved(int i, int c = 1)
        {
            ...
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            int rc = Native.ikat_list_notify(h, _id, (byte)1, i, c);
            if (rc != 0)
                throw new UIContractException(
        ...
        public void NotifyMoved(int f, int t)
        {
            ...
            StageHandle* h = (StageHandle*)_ctx._stage.ToPointer();
            int rc = Native.ikat_list_notify(h, _id, (byte)2, f, t);
            if (rc != 0)
                throw new UIContractException(
    ...
    public sealed unsafe class AnimationHandle
    {
        ...
        public bool IsPlaying
        {
            get
            {
                ...
                byte state = Native.ikat_stage_get_animation_state(h, _playerKey);
                if (state == 255)
                {
        ...
        public float Time
        {
            get
            {
                ...
                StageHandle* h = (StageHandle*)_node._ctx._stage.ToPointer();
                return Native.ikat_stage_get_animation_time(h, _playerKey);
            }
            set
            {
                if (_disposed || _node._disposed) return;
                StageHandle* h = (StageHandle*)_node._ctx._stage.ToPointer();
                Native.ikat_stage_set_animation_time(h, _playerKey, value);
            }
        }
        ...
        public void Pause()
        {
            if (_disposed || _node._disposed) return;
            StageHandle* h = (StageHandle*)_node._ctx._stage.ToPointer();
            Native.ikat_stage_pause_animation(h, _playerKey);
        }
        ...
        public void Resume()
        {
            if (_disposed || _node._disposed) return;
            StageHandle* h = (StageHandle*)_node._ctx._stage.ToPointer();
            Native.ikat_stage_resume_animation(h, _playerKey);
        }
        ...
        public void Stop()
        {
            if (_disposed || _node._disposed) return;
            StageHandle* h = (StageHandle*)_node._ctx._stage.ToPointer();
            Native.ikat_stage_stop_animation(h, _playerKey);
            Invalidate();
        }
        ...
        public AnimationHandle OnKey(float pct, Action cb)
        {
            ...
            if (_disposed || _node._disposed) return this;
            StageHandle* h = (StageHandle*)_node._ctx._stage.ToPointer();
            Native.ikat_stage_animation_on_key(h, _playerKey, pct);
        ...
        /// <summary>
        /// 链式注册 @ikat-hook 锚点回调（纯 C#：core emit HOOK 带 hook_name，
    ...
    public sealed unsafe class ClassList
    {
        ...
        void CallAdd(string name)
        {
            ...
            fixed (byte* p = b)
                Native.ikat_stage_add_class(h, _owner._id, p, (nuint)b.Length);
        }
        ...
        void CallRemove(string name)
        {
            ...
            byte[] b = Encoding.UTF8.GetBytes(name);
            fixed (byte* p = b)
                Native.ikat_stage_remove_class(h, _owner._id, p, (nuint)b.Length);
        }
        ...
        int CallHas(string name)
        {
            ...
            byte[] b = Encoding.UTF8.GetBytes(name);
            fixed (byte* p = b)
                return Native.ikat_stage_has_class(h, _owner._id, p, (nuint)b.Length);
        }
    }
    ...
    public sealed unsafe class UITemplate
    {
        ...
        internal static Container DoInstantiateSubtree(UIContext ctx, ulong srcNodeId)
        {
            ...
            ulong rootId = Native.ikat_stage_clone_subtree(h, srcNodeId);
        ...
        internal static Container DoInstantiate(UIContext ctx, string pkg, string path)
        {
            ...
            fixed (byte* pp = pb)
            fixed (byte* cp = cb)
                rootId = Native.ikat_stage_instantiate(h, pp, (nuint)pb.Length, cp, (nuint)cb.Length);
            if (rootId == Node.RootSentinel)
                throw new UIPackageException(
    ...
    public sealed unsafe class TweenBuilder
    {
        ...
        public void Start()
        {
            ...
            if (_spec.prop == (uint)TweenChannel.BoxShadow)
            {
                ...
                IkatTweenSpec spec = _spec;
                fixed (float* sPtr = sp)
                fixed (float* ePtr = ep)
                {
                    Native.ikat_stage_tween_shadow(h, _node._id, &spec,
            ...
            {
                IkatTweenSpec spec = _spec;
                Native.ikat_stage_tween_spec(h, _node._id, &spec, sp, ep);
            }
        }
    ...
    public sealed unsafe class UIContext
    {
        ...
        // 投影层通过它转调 ikat_stage_* FFI；公共 API 表面看不到本字段。
        ...
        // EventDemuxer 翻译 raw IkatEvent → typed struct 后调 Dispatch<T>。公共 API 不见本字段。
        internal readonly EventBus _eventBus;

        // raw IkatEvent stream → typed event struct demux。IkatHost.Step 调 Pump 每帧
        ...
        // tick 后队列（CallAfterLayout）：IkatHost.Step 在 stage tick 之后泵——新挂载
        ...
        /// IkatHost.Step 在 tick 前调（flush→solve 序）；headless 测试在 raw tick 前调。
        ...
        /// IkatHost.Step 在 CollectInput 后、FlushPendingWrites 前调——回调内改 Style 走
        ...
        void InvokeLogicGuarded(Action cb)
        {
            ...
            catch (Exception ex)
            {
                ...
                System.Diagnostics.Debug.WriteLine($"[Ikat] logic callback threw: {ex}");
            }
        }

        /// <summary>
        /// tick 后泵：排空 after-layout 回调。IkatHost.Step 在 stage tick 之后调——
        ...
        internal void DrainPendingBinds()
        {
            ...
            uint len = 0;
            int rc = Native.ikat_list_take_pending_binds(h, nodes, indices, Cap, &len);
            if (rc != 0) return;
            for (int i = 0; i < len; i++)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    System.Diagnostics.Debug.WriteLine(
                        $"[Ikat] ListView BindItem threw for item {itemIndex} (slot node {slotNode}): {ex}");
                }
            }
        }
        ...
        ListView FindListViewAncestor(StageHandle* h, ulong slotNode)
        {
            ...
            for (int i = 0; i < 100_000; i++)
            {
                ...
                cur = Native.ikat_node_parent(h, cur);
            }
            return null;
        }
        ...
        /// <summary>
        /// 当前焦点节点。FFI ikat_stage_focused_node 返 NodeId（无焦点 → sentinel）。
        /// 返 null 当无焦点（DOM document.activeElement 为 body 的习惯：Ikat 返 null 而非抛异常）。
        /// </summary>
        public Node FocusedNode
        {
            get
            {
                ...
                StageHandle* h = (StageHandle*)_stage.ToPointer();
                ulong id = Native.ikat_stage_focused_node(h);
        ...
        public TextMetrics MeasureText(string text, string fontFamily, float sizePx, float maxWidth = 0f)
        {
            ...
            fixed (byte* tp = tb)
            fixed (byte* fp = fb)
                rc = Native.ikat_stage_measure_text(
            ...
            switch (rc)
            {
                ...
                case -2:
                    throw new UIContractException(
                        ...
                        "IkatHost.RegisterFont (or the runtime manifest) first — measure must use " +
        ...
        public UIPackage LoadPackage(string name, byte[] bytes)
        {
            ...
            fixed (byte* np = nb)
            fixed (byte* bp = bytes)
                rc = Native.ikat_stage_load_package(h, np, (nuint)nb.Length, bp, (nuint)bytes.Length);
            if (rc != 0)
            {
                // 版本错配（rc 1/2）专属文案：Unity 包与 ikat.exe CLI 必须同版本同刷——
                // 只升一侧时旧 CLI 打的 pkg 被新运行时拒载（或反之），报错须点破修法。
                if (rc == 1 || rc == 2)
                {
                    uint pkgVer = Native.ikat_stage_last_pkg_load_version(h);
                    uint runtimeVer = Native.ikat_pkg_format_version();
                    string dir = rc == 1 ? "older" : "newer";
                    throw new UIPackageException(
                        $"load_package '{name}' failed: pkg format v{pkgVer} is {dir} than this " +
                        $"runtime's v{runtimeVer}. The Unity package and the ikat.exe CLI must be " +
                        $"upgraded together — re-run `ikat build` with the ikat.exe shipped with this " +
                        $"package version (check `.ikat/ikat.exe --version`).");
                }
                throw new UIPackageException(
        ...
        public void UnloadPackage(string name)
        {
            ...
            int rc;
            fixed (byte* np = nb)
                rc = Native.ikat_stage_unload_package(h, np, (nuint)nb.Length);
            if (rc != 0)
        ...
        public T Create<T>() where T : Node
        {
            ...
            ulong id;
            fixed (byte* tp = tb)
                id = Native.ikat_stage_create_node(h, tp, (nuint)tb.Length, null, 0);
        ...
        /// 直转 ikat_stage_hit_test（core hit::hit_test，上帧 world_transforms；结构
        ...
        public Node Pick(IkatVector2 globalPoint)
        {
            StageHandle* h = (StageHandle*)_stage.ToPointer();
            ulong id = 0;
            int rc = Native.ikat_stage_hit_test(h, globalPoint.X, globalPoint.Y, &id);
            if (rc == 1) return null;
        ...
        /// 直透传 ikat_stage_is_pointer_on_ui FFI。
        ...
        public bool IsPointerOnUI
        {
            get
            {
                ...
                StageHandle* h = (StageHandle*)_stage.ToPointer();
                return Native.ikat_stage_is_pointer_on_ui(h);
            }
        }
