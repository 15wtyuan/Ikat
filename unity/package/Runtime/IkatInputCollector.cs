...
namespace Ikat
{
    ...
    [ExecuteAlways]
    public unsafe class IkatInputCollector : MonoBehaviour
    {
        /// <summary>
        /// 画布尺寸 + safe-area 开关 + 适配映射三元组（MapScale/MapOffX/MapOffYTopDown）：
        /// screen→design 映射消费三元组（单源 = Rust ikat_compute_adaptation，Driver 每次适配
        ...
        /// screen→design 映射，消费 Driver 注入的适配三元组（单源 Rust ikat_compute_adaptation，
        ...
        public void Collect(System.IntPtr stage)
        {
            ...
            if (events.Count == 0)
            {
                Native.ikat_stage_set_input((Bindings.StageHandle*)stage, null, 0);
                return;
            }
            ...
            {
                Native.ikat_stage_set_input((Bindings.StageHandle*)stage, p, (nuint)arr.Length);
            }
        }
        ...
        public void CollectKeys(System.IntPtr stage)
        {
            ...
            {
                Native.ikat_stage_set_key_input((Bindings.StageHandle*)stage, p, (nuint)arr.Length);
            }
        }
        ...
        public void CollectText(System.IntPtr stage)
        {
            ...
            {
                Native.ikat_stage_set_text_input((Bindings.StageHandle*)stage, null, 0);
                return;
            }
            ...
            fixed (uint* p = arr)
            {
                Native.ikat_stage_set_text_input((Bindings.StageHandle*)stage, p, (nuint)arr.Length);
            }
        }
        ...
        /// UnityEngine.GUI TextField，Ikat 自绘不触发，故须显式 On），失焦关 IME。
        ...
        public void CollectComposition(System.IntPtr stage)
        {
            ...
            var kb = Keyboard.current;
#endif
            ulong focused = Native.ikat_stage_focused_node(h);
            ...
            if (Native.ikat_stage_get_cursor_rect(h, focused, &r) == 0)
            {
            ...
            if (bytes.Length == 0)
            {
                Native.ikat_stage_set_composition(h, focused, null, 0, 0);
            }
            else
            {
                fixed (byte* p = bytes)
                {
                    Native.ikat_stage_set_composition(h, focused, p, (nuint)bytes.Length, 0);
                }
            }
        }
        ...
        public static void CollectWheel(System.IntPtr stagePtr, IkatInputCollector ctx)
        {
            ...
            Native.ikat_stage_set_wheel_input((Bindings.StageHandle*)stagePtr, &ev, 1);
        }
