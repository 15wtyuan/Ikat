...
namespace Ikat
{
    ...
    /// 引擎无关抽象基类——零 UnityEngine（放 Runtime/Host/，随 Ikat.Runtime asmdef 编译）。
    /// Unity 实现 <see cref="UnityIkatBackend"/>；Godot-C# 未来实现 GodotIkatBackend。
    /// <see cref="IkatHost"/> 持具体实现并按每帧管线驱动。
    /// </summary>
    public abstract class IkatBackend
    {
        /// <summary>
        ...
        /// </summary>
        /// <param name="stage">Stage handle（<see cref="IkatHost"/> 持有的 native StageHandle*，以 IntPtr 透传）。</param>
        public abstract void CollectInput(IntPtr stage);

        /// <summary>
        /// 消费 borrow_frame 拿到的 frame blob 做镜像渲染——本方法不调 borrow FFI
        /// （<see cref="IkatHost"/> 已 borrow，把 stage + ptr + len 传进来，避免二次 borrow）。
