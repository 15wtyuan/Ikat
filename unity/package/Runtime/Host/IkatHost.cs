// IkatHost：引擎无关 stage 宿主 + 每帧驱动核心。
//
// 设计契约（严格时序）：
// - 持 stage handle（StageHandle*）+ UIContext（业务表面）+ IkatBackend（Unity/Godot 实现）。
// - 构造：ikat_stage_new → UIContext(stage)（复用 internal UIContext(IntPtr)）。
...
// - Dispose：ikat_stage_free。
//
// 零 UnityEngine（放 Runtime/Host/）。Unity + Godot-C# 共享——Godot 写 GodotIkatBackend : IkatBackend 注入。
...
namespace Ikat
{
    ...
    /// <see cref="IkatBackend"/>。Driver 持本类，每帧调 <see cref="Step"/>。
    /// 零 UnityEngine（放 Runtime/Host/，Unity+Godot-C# 共享）。
    /// </summary>
    public sealed unsafe class IkatHost : IDisposable
    {
        StageHandle* _stage;
        readonly UIContext _ctx;
        readonly IkatBackend _backend;
        ...
        /// <summary>
        /// 每帧驱动序（严格时序）：
        /// 1. <see cref="IkatBackend.CollectInput"/>（backend 采集引擎输入 → set_input 系 FFI，引擎中立）
        ...
        /// 3. ikat_stage_tick（核心 process/rematch/solve/refresh/compute_world/build）
        /// 4. borrow_frame → <see cref="IkatBackend.SyncFrame"/>（backend 只消费 blob，不再调 borrow FFI）
        ...
        public void Step(float dt)
        {
            ...
            Native.ikat_stage_tick(_stage, dt);
            ...
            nuint warnLen = 0;
            byte* warnPtr = Native.ikat_stage_take_warnings(_stage, &warnLen);
            ...
            nuint lenRaw = 0;
            byte* ptr = Native.ikat_stage_borrow_frame(_stage, &lenRaw);
            ...
            //    UIContext._eventDemuxer 在 ctx 构造时建并接到 ctx._eventBus——IkatHost 复用单一实例。
            nuint evLen = 0;
            byte* evPtr = Native.ikat_stage_borrow_events(_stage, &evLen);
        ...
        public void SetFallbackFamilies(IEnumerable<string> families)
        {
            ...
            fixed (byte* tp = tb)
            {
                Native.ikat_stage_set_fallback_families(_stage, tp, (nuint)tb.Length);
            }
        }
        ...
        /// 实际 Texture2D 上传由 <see cref="UnityIkatBackend.InitSprites"/> 走 SpriteResolver 注册。
        /// </summary>
        public void SetImageSizes(string[] paths, uint[] ws, uint[] hs)
        {
            ...
            try
            {
                ...
                fixed (uint* hp = hs)
                {
                    Native.ikat_stage_set_image_sizes(_stage, (byte**)pp, wp, hp, (nuint)n);
                }
            }
            finally
            {
        ...
        /// <summary>
        /// dump 整树 JSON（调 <see cref="Native.ikat_stage_dump_scene"/>，UTF-8 marshal）。
        /// Rust 侧拥有 C 串、下 tick 失效——立即消费。未 instantiate（scene=None）/ 已 Dispose → "[]"。
        /// dev 调试桥用（unity-cli-loop execute-dynamic-code 经 Showcase.IkatBridge 调），非冻结公共签名。
        /// </summary>
        public string DumpSceneJson()
        {
            if (_stage == null) return "[]";
            nuint len = 0;
            byte* ptr = Native.ikat_stage_dump_scene(_stage, &len);
        ...
        public string DumpSceneTree(string filter = null)
        {
            ...
            byte* ptr;
            fixed (byte* fp = fb)
                ptr = Native.ikat_stage_dump_tree(_stage, fp, (nuint)(fb?.Length ?? 0), &len);
