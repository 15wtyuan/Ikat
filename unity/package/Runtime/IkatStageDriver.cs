...
{
    /// <summary>
    /// IkatHost 的 Unity 生命周期宿主。持引擎无关 <see cref="IkatHost"/>（stage 句柄 +
    /// <see cref="UIContext"/> + <see cref="IkatBackend"/>）+ Unity 特定 <see cref="UnityIkatBackend"/>
    ...
    /// <see cref="IkatHost.Step"/>（内含 CollectInput→tick→borrow_frame→SyncFrame→borrow_events→Pump）。
    ///
    /// 启动流程：读 ikat.runtime.json → 加载包 → 加载 atlas.json → set_image_sizes →
    /// SpriteResolver.Init → 注册字体 → 正常 tick。不再依赖 ScriptableObject 配置（改读 ikat.runtime.json）。
    ///
    ...
    public class IkatStageDriver : MonoBehaviour
    {
        ...
        /// 持 stage 句柄 + <see cref="UIContext"/> + <see cref="UnityIkatBackend"/>。
        /// Awake 失败时为 null。
        /// </summary>
        public IkatHost Host => _host;
        ...
        public virtual Texture2D LoadTexture(string relPath)
        {
            ...
            {
                Debug.LogWarning($"[IkatStageDriver] Failed to load texture {path}: {e.Message}");
                return null;
            }
        }
        ...
        void Awake()
        {
            // ExecuteAlways：EditMode/Play 反复 Awake + domain reload 会让上一轮的 ikat_node 镜像 GO
            // （root 的子）成孤儿残留——上一轮 _pool 引用已丢、Clear 不到。开局先清 root 下所有 ikat_node
            // 子 GO，防累积泄漏。UI 相机是独立 GO（SetParent(null)），非 root 子，不受影响。
            for (int c = transform.childCount - 1; c >= 0; c--)
            {
                var child = transform.GetChild(c);
                if (child.name == "ikat_node") DestroyImmediate(child.gameObject);
            }
            ...
            if (shader == null)
            {
                Debug.LogError("[IkatStageDriver] Shader Ikat/Unlit not found");
                return;
            }
            ...
            {
                Debug.LogError($"[IkatStageDriver] IkatHost construct failed: {e.Message}");
                return;
            }
            ...
            _host.MissingGlyphReport += msg => Debug.LogWarning($"[Ikat] missing glyphs (tofu):\n{msg}");
            _host.RuntimeWarning += msg => Debug.LogWarning($"[Ikat] {msg}");
#endif
            ...
            if (runtime != null)
            {
                ...
                foreach (var pkgName in runtime.packages)
                {
                    ...
                    if (bytes != null && bytes.Length > 0)
                        ...
                        Debug.LogWarning($"[IkatStageDriver] Package not found: ui/{pkgName}.pkg.bin");
                }
                ...
                foreach (var atlasName in runtime.atlases)
                {
                    ...
                    {
                        Debug.LogWarning($"[IkatStageDriver] atlas.json not found: atlas/{atlasName}.atlas.json");
                        continue;
                    }
                    try { atlasManifests.Add(AtlasManifest.ParseAtlas(atlasJson)); }
                    catch (Exception e) { Debug.LogWarning($"[IkatStageDriver] Failed to parse atlas/{atlasName}.atlas.json: {e.Message}"); }
                }
                ...
                //    Unity 特定资源 IO（Texture2D）——归 UnityIkatBackend，不进引擎无关 IkatHost。
        ...
        unsafe void EnsureSceneRoot()
        {
            ...
            ulong rootId;
            fixed (byte* kp = kind)
                rootId = Native.ikat_stage_create_root(h, kp, (nuint)kind.Length, null, 0);
            if (rootId == Node.RootSentinel)
            {
                Debug.LogError("[IkatStageDriver] create_root failed (stage null / kind non-UTF-8)");
                return;
            }
            ctx._rootId = rootId;
        }
        ...
        ///
        /// pkgName 必须已在 ikat.runtime.json packages 段列出（Awake 时已 load_package）。
        ...
        public unsafe Container Instantiate(string pkgName, string compPath)
        {
            if (_host == null)
            {
                Debug.LogError("[IkatStageDriver] Instantiate called but host is null (Awake failed?)");
                return null;
            }
            EnsureSceneRoot();
            var ctx = _host.Context;
            ...
            {
                Debug.LogError($"[IkatStageDriver] Instantiate({pkgName},{compPath}) aborted: scene root not created");
                return null;
            }
            ...
            fixed (byte* pp = pb)
            fixed (byte* cp = cb)
                instId = Native.ikat_stage_instantiate(h, pp, (nuint)pb.Length, cp, (nuint)cb.Length);
            if (instId == Node.RootSentinel)
            {
                Debug.LogError($"[IkatStageDriver] instantiate failed: pkg={pkgName} comp={compPath} (pkg not loaded / comp not found / scene missing)");
                return null;
            }
        ...
        void DumpDiagnostic()
        {
            ...
            try
            {
                ...
                string path = Path.Combine(dir, $"ikat-dump-{stamp}.txt");
        ...
        /// <summary>
        /// dev 调试桥用：返回 MirrorPool 状态文本（转发 <see cref="UnityIkatBackend.DumpMirrorState"/>，
        /// 同 F8 诊断源）。PlayMode 下有活跃 backend；无 driver/backend 时返提示串。被
        /// Showcase.IkatBridge.DumpMirrorPool 经 unity-cli-loop execute-dynamic-code 调。
        /// </summary>
...
#if UNITY_EDITOR || DEVELOPMENT_BUILD
        // F9 pick 命中链探针（编辑器/开发构建；IkatDebugProbe.DescribePickChain 本体
        ...
        void UpdatePickProbe()
        {
            ...
            if (kb != null && kb.f9Key.wasPressedThisFrame)
            {
                ...
                Debug.LogWarning($"[Ikat] pick probe {(_pickProbeOn ? "ON" : "OFF")} (F9)");
            }
            ...
            var design = IkatInputCollector.ScreenToDesign(
                screen, _adaptScale, _adaptOffX, _adaptOffYTopDown, Screen.height);
            Node hit = _host.Context.Pick(new IkatVector2(design.x, design.y));
            ulong hitId = hit?._id ?? ulong.MaxValue;
            if (hitId == _probeLastHit) return;
            _probeLastHit = hitId;
            Debug.LogWarning(IkatDebugProbe.DescribePickChain(_host.Context, design.x, design.y));
#endif
        }
#endif
        ...
        /// <see cref="IkatInputCollector.ScreenToDesign"/> 用同一组 sf/偏移逐项逆映射，保触摸↔渲染对齐。
        /// </summary>
        ...
        /// （ikat_compute_adaptation，策略单源——未来 Godot 后端复用同一份）算画布 + 映射三元组。
        ...
        void ResolveAdaptation(RuntimeManifest runtime, bool applyToStage)
        {
            ...
            {
                var m = Ikat.Bindings.IkatAdaptMode.FromString(runtime.match_mode);
                if (m.HasValue) _modeU32 = m.Value;
                else Debug.LogWarning($"[IkatStageDriver] unknown match_mode '{runtime.match_mode}' in ikat.runtime.json -> Inspector fallback ({_adaptMode})");
            }
        ...
        unsafe void RecomputeAdaptation(float dw, float dh, bool applyToStage)
        {
            ...
            if (Native.ikat_compute_adaptation(
                    dw, dh, sw, sh, a.x, sh - (a.y + a.height), a.width, a.height, _modeU32, &r) != 0)
            {
                Debug.LogError("[IkatStageDriver] ikat_compute_adaptation failed -> fallback letterbox @design");
