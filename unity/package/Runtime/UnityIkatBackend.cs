...
namespace Ikat
{
    ...
    /// IkatInputCollector。<see cref="IkatHost"/> 通过
    /// <see cref="IkatBackend"/> 契约驱动：每帧先 <see cref="CollectInput"/> 再 <see cref="SyncFrame"/>
    /// （borrow_frame 已由 IkatHost 完成，ptr+len 传入避免二次 borrow）。
    ///
    /// NativeHost（GameObject 绑定 3D 模型，<see cref="NativeHostManager"/>）是 Unity 专属，
    /// 不进 <see cref="IkatBackend"/> 通用契约，作额外属性 <see cref="NativeHost"/> 暴露给 IkatHost。
    ///
    ...
    /// ArrayPool frame buffer。<see cref="IkatHost.Dispose"/> 只释放 stage 句柄（引擎中立），
    /// 引擎资源归本类。
    /// </summary>
    public sealed unsafe class UnityIkatBackend : IkatBackend, IDisposable
    {
        ...
        internal SpriteResolver _sprites = new();  // IkatHost InitSprites/SetImageSizes 资源注册用
        IkatInputCollector _inputCollector;  // Driver Awake 注入（指针/键盘/滚轮采集）
        ...
        /// </summary>
        public void SetRuntimeRoot(Transform root, IkatInputCollector input)
        {
            _renderRoot = root;
        ...
        /// <summary>
        /// NativeHost 绑定点（Unity 专属，不进 IkatBackend 通用契约）。
        /// internal——<see cref="NativeHostManager"/> 自身是 internal sealed，IkatHost 同程序集可见。
        /// </summary>
        ...
        ///
        /// Unity 特定资源 IO（Texture2D）——不进 <see cref="IkatHost"/> 引擎无关层。
        /// </summary>
        ...
        public override void CollectInput(IntPtr stage)
        {
            ...
            IkatInputCollector.CollectWheel(stage, _inputCollector);
        }
        ...
        {
            uint count = Native.ikat_ffi_panic_count();
            if (count == _lastFfiPanicCount) return;
            Debug.LogError($"[Ikat] native panic caught by FFI guard (total {count}); frame state may be inconsistent — check native log for panic origin");
        ...
        unsafe void SyncFontAtlas(StageHandle* h)
        {
            ...
            int n = (int)Native.ikat_stage_font_atlas_dirty_pages(h, dirtyPtr, (nuint)MAX_DIRTY);
            if (n <= 0) return;
            if (n > MAX_DIRTY)
            {
                Debug.LogWarning($"[UnityIkatBackend] font atlas dirty pages ({n}) exceed MAX_DIRTY ({MAX_DIRTY}); skipping extras");
                n = MAX_DIRTY;
            }
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                int needed = (int)Native.ikat_stage_font_atlas_page(h, page, &w, &ph, null, (nuint)0);
                ...
                try
                {
                    fixed (byte* pBuf = buf)
                    {
                        int got = (int)Native.ikat_stage_font_atlas_page(h, page, &w, &ph, pBuf, (nuint)needed);
                    ...
                    // build_text_mesh 合成的 ikat://font-atlas/p{n} 对齐。
                    string path = FontAtlasPath.Format(page);
                    _sprites.RegisterFontAtlasPage(path, tex);
                }
                finally { ArrayPool<byte>.Shared.Return(buf); }
            }
            Native.ikat_stage_font_atlas_clear_dirty(h);
        }
        ...
        /// SpriteResolver 缓存 + ArrayPool frame buffer。<see cref="IkatHost"/>.Dispose 不递归——
