...
namespace Yio
{
    ...
    /// 按名认领（"YioUICamera"）先于新建：编辑器脚本重编译不跑 OnDestroy、自建相机
    ...
    /// 文档写明；重叠拖拽属边缘场景）。
    /// </summary>
    public static class YioStageHub
    {
        ...
        const string CameraName = "YioUICamera";

        static readonly List<(YioStageDriver driver, int order, int seq)> s_drivers = new();
        ...
        /// </summary>
        public static int Register(YioStageDriver driver, int order)
        {
        ...
        public static void Unregister(YioStageDriver driver)
        {
        ...
        public static int SortBaseOf(YioStageDriver driver)
        {
            ...
            {
                Debug.LogWarning(
                    $"[YioStageHub] {idx + 1} stages exceed sortingOrder budget ({MaxOrdinals} x {SortStride}); stage '{driver.name}' shares the top band with stage {MaxOrdinals - 1}");
                idx = MaxOrdinals - 1;
            }
        ...
        /// 合法 YioUICamera 只有两种形态：本次共享相机本体（跳过）、其它已加载场景里
        ...
        /// </summary>
        public static YioStageDriver RouteInput(YioStageDriver self)
        {
        ...
        /// <summary>domain reload 清（Driver.ResetStatics 调；同 yio_shutdown 钩）。</summary>
