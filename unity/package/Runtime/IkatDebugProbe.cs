...
namespace Ikat
{
    ...
    /// </summary>
    public static unsafe class IkatDebugProbe
    {
        /// <summary>
        /// 返回 (x, y)（design 坐标，左上原点）处的 Pick 命中链描述文本。未命中返回 miss 行。
        /// Pick 用上帧 world_transforms（结构变更帧的新节点 1 帧延迟命中）。逐帧调用会刷屏——
        /// 调用方按顶层命中变化去重后再打日志（<see cref="IkatStageDriver"/> 的 F9 探针已去重）。
        /// </summary>
        public static string DescribePickChain(UIContext ctx, float x, float y)
        {
            Node hit = ctx.Pick(new IkatVector2(x, y));
            if (hit == null)
                return $"[Ikat pick probe] miss at ({x:F0},{y:F0}) — no touchable node under pointer";

            var sb = new StringBuilder(256);
            sb.AppendLine($"[Ikat pick probe] hit at ({x:F0},{y:F0})");
            int level = 0;
            for (Node n = hit; n != null; n = n.Parent, level++)
            {
                IkatRect wr = n.Geometry.WorldRect;
                sb.AppendLine(
        ...
        static float OpacityOf(Node n)
        {
            ...
            float v = 1f;
            int rc = Native.ikat_stage_get_node_opacity(h, n._id, &v);
            return rc == 0 ? v : 1f;
        }
        ...
        static string ClassesOf(Node n)
        {
            ...
            {
                int rc = Native.ikat_stage_get_node_classes(h, n._id, sbp, (nuint)stackBuf.Length, &needed);
            ...
            {
                nuint written = 0;
                int rc = Native.ikat_stage_get_node_classes(h, n._id, hbp, (nuint)heapBuf.Length, &written);
                if (rc != 0) return "?";
