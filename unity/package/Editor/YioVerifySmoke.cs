...
{
    /// <summary>
    /// `yio verify` 的 Unity 侧入口（batchmode 导入冒烟）。
    /// CLI 拉起形如：
    ///   Unity -batchmode -quit -nographics -projectPath &lt;root&gt;
    ///     -executeMethod Yio.Editor.YioVerifySmoke.Run -logFile &lt;tmp&gt;
    ///     -yioVerifyDir=Assets/&lt;output_dir&gt; -yioVerifyReport=&lt;tmp&gt;
    /// 语义：Refresh 全量导入后对产物目录逐文件**正向加载**（png → Texture2D
    /// 解码证明；其余 → 非空 Object 导入证明），报告写 -yioVerifyReport 文件
    ...
    /// </summary>
    public static class YioVerifySmoke
    {
        public static void Run()
        {
            string dir = FindArg("-yioVerifyDir=");
            string reportPath = FindArg("-yioVerifyReport=");
            if (string.IsNullOrEmpty(dir) || string.IsNullOrEmpty(reportPath))
            {
                UnityEngine.Debug.LogError(
                    "[Yio][verify] missing -yioVerifyDir= / -yioVerifyReport= args");
