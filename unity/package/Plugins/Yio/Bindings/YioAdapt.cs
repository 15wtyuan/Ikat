...
namespace Yio.Bindings
{
    /// 手补 C# 镜像（csbindgen 不为 use-imported 的 Rust #[repr(C)] struct 生成 stub
    /// ——只扫描 lib.rs 内 #[no_mangle] fn 签名，不追 use 路径；同 YioKeyEvent.cs 模式）。
    /// AdaptResult 被 yio_compute_adaptation 签名引用但无 stub → 手动补。
    /// #[repr(C)] 5 × f32 = 20B，字段序须与 Rust yio_core::adapt::AdaptResult 一致。
    /// </summary>
    ...
    /// 字符串形态（yio.workspace.json / yio.runtime.json 的 match_mode）：
    /// letterbox | fit-width | fit-height。
    /// </summary>
    internal static class YioAdaptMode
    {
