...
namespace Yio.Bindings
{
    ...
    /// PointerEvent 被 yio_stage_set_input 签名引用但无 stub → 手动补，放 Yio.Bindings
    /// 程序集（与 Native/StageHandle 同集，签名可解析）。
    ///
    /// 字段序与 Rust yio_core::input::PointerEvent（#[repr(C)], PointerKind repr(u8)）一致：
    ...
    /// 与 Rust repr(C) 一致（+touch_id，PointerKind repr(u8) 1B 判别）。
    ...
    internal unsafe struct PointerEvent
    {
        ...
        public byte pad1;
        public int touch_id;     // -1=鼠标主指，>=0=触摸 fingerId
        public float x;
        public float y;
    }
}
