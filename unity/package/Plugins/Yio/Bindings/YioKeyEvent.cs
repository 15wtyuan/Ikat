...
namespace Yio.Bindings
{
    ...
    /// ——只扫描 lib.rs 内 #[no_mangle] fn 签名，不追 use 路径；同 YioPointerEvent.cs 模式）。
    /// KeyEvent 被 yio_stage_set_key_input 签名引用但无 stub → 手动补。
    ///
    /// 字段序与 Rust yio_core::input::KeyEvent（#[repr(C)]）一致：
    ...
    /// 字段名（key_code/modifiers/is_down/pad0/pad1）与 YioInputCollector.CollectKeys 消费侧一致。
