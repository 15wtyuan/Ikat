...
namespace LoomGUI.Bindings
{
    ...
    /// ——只扫描 lib.rs 内 #[no_mangle] fn 签名，不追 use 路径；同 LoomGUIKeyEvent.cs 模式）。
    /// AdaptResult 被 loomgui_compute_adaptation 签名引用但无 stub → 手动补。
    /// #[repr(C)] 5 × f32 = 20B，字段序须与 Rust loomgui_core::adapt::AdaptResult 一致。
    /// </summary>
    ...
    /// 字符串形态（loom.workspace.json / loom.runtime.json 的 match_mode）：
    /// letterbox | fit-width | fit-height。
    /// </summary>
    internal static class LoomAdaptMode
    {
