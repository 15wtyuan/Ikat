...
{
    /// <summary>
    /// YioStageDriver EditMode tests. Unity PlayMode/EditMode tests cannot run from CLI
    /// (require Unity Editor), executed via Unity Test Runner during acceptance.
    /// Here we verify compilation + pure-logic correctness.
    /// </summary>
    public class YioStageDriverTests
    {
        /// <summary>
        /// Driver.Awake must construct YioHost instance (Context property non-null).
        /// Verifies Awake sequence: new UnityYioBackend + YioHost + SetRuntimeRoot +
        /// NativeHost.Init + bootstrap from runtime.json + EnsureCamera + ConfigureTransforms
        /// all run without exception.
        /// </summary>
        [Test]
        public void YioStageDriver_AwakeBuildsHost()
        {
            ...
            try
            {
                ...
                Assert.IsNotNull(driver.Context, "Driver.Awake must construct YioHost (Context non-null)");
            }
            finally
            {
        ...
        {
            var result = YioStageDriver.MergeSpriteSizes(null);
        ...
        {
            var result = YioStageDriver.MergeSpriteSizes(new List<AtlasManifest>());
            Assert.IsNotNull(result);
            Assert.AreEqual(0, result.Count);
        }
        ...
        public void MergeSpriteSizes_SingleAtlas_ReturnsSprites()
        {
            ...
            var result = YioStageDriver.MergeSpriteSizes(new List<AtlasManifest> { atlas });
        ...
        public void MergeSpriteSizes_DuplicateKeys_FirstWins()
        {
            ...
            var result = YioStageDriver.MergeSpriteSizes(new List<AtlasManifest> { atlas1, atlas2 });
            Assert.AreEqual(2, result.Count);
