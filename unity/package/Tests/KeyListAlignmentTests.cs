...
{
    /// CollectKeys 新旧路径的平行键表对齐校验：KeyList（KeyCode）与 NewKeyList
    ...
    {
#if ENABLE_INPUT_SYSTEM
        /// KeyCode 名 → Key 名的仅有差异：Alpha0-9↔Digit0-9、Return↔Enter。
        ...
        public void ParallelKeyTablesAligned()
        {
            ...
            for (int i = 0; i < LoomInputCollector.KeyList.Length; i++)
            {
                ...
                Assert.AreEqual(
                    ...
                    $"下标 {i}：KeyCode.{LoomInputCollector.KeyList[i]} 应配 Key.{expected}");
            }
        }
#endif
    }
