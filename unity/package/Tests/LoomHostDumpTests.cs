...
{
    /// <summary>
    /// LoomHost.DumpSceneJson EditMode tests——验证 loomgui_stage_dump_scene 的 byte*→UTF-8 marshal
    /// （含 NUL 终止符剥离）+ null/disposed 守卫。host 经 LoomStageDriver.Awake 构造（同
    /// LoomStageDriverTests 先例），不直接 new LoomHost（构造要 backend）。PlayMode 全量由 acceptance 跑。
    /// </summary>
