...
消费者路径验收样板：干净 URP 工程 + 本地安装 `com.loomgui.unity`（file: 依赖），
...
- `Assets/Scenes/SampleScene`：`LoomUI` GO（LoomStageDriver 1080×1920→**1920×1080** + LoomInputCollector + ShowcaseRunner）
...
或用打包器重新产出：`cargo run -p loomgui_pkg -- build <workspace>`。
