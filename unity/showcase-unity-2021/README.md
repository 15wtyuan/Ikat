...
消费者路径验收样板：干净 URP 工程 + 本地安装 `com.yio.unity`（file: 依赖），
...
- `Assets/Scenes/SampleScene`：`YioUI` GO（YioStageDriver 1080×1920→**1920×1080** + YioInputCollector + ShowcaseRunner）
...
或用打包器重新产出：`cargo run -p yio_pkg -- build <workspace>`。
