...
namespace LoomGUI
{
    ...
    public sealed unsafe class LoomStage : MonoBehaviour
    {
        ...
        // Sprite Atlas 接入。开发者建 SpriteAtlas asset（把 res/ 下 Sprite 划进去），
        ...
        void Awake()
        {
            ...
            // path→Sprite 查询。注册 Inspector 配的 SpriteAtlas。
        ...
        void OnDestroy()
        {
            ...
            _nhm?.Clear();
            _mm?.Clear();
            // SpriteResolver 是纯缓存（Dictionary + List<SpriteAtlas>），无 UnityEngine.Object
