...
**重出无判据——`cargo run -p xtask -- reout` 一条命令全家桶**（dll + ikat.exe + ikat_gui.exe + bindings + showcase bundle + HeadlessTests fixture 包）。GUI exe 与其余产物无条件同批重出、同 commit 提交：GUI 直链 ikat_pkg/ikat_fence（dev fallback 进程内路径），任何 pkg/fence 改动都编进 exe 字节——旧的「仅 Workspace struct / GUI 自身代码变动才重出」收窄判据漏掉这条内嵌路径且本身是心智负担，已废除。GUI 变动同样计入 staleness 硬门（release-check / CI）。手工兜底（xtask 不可用时）：
...
- **本地 headless 工程不编 Runtime/ 层（编译门盲区，两次实锤）**——本地 `dotnet test` 编的是 Tests.Core 子集；headless 双工程（PublicApi/HeadlessTests）也只链 `Runtime/Public/ + Projection/ + Bindings/`，**顶层 Runtime 文件（IkatStageDriver/MirrorPool/FrameBlob/IkatInputCollector/NativeHostManager 等，引 UnityEngine）只有用户 Unity 才编**。改这些文件后收尾必跑全部三个 CI 工程（Tests.Core 也算——CI 的 pure managed 门会跑它，本批漏跑即 CI 红）：`dotnet test tests/dotnet/Ikat.Tests.Core.csproj` + `dotnet build tests/dotnet/Ikat.PublicApi/Ikat.PublicApi.csproj -warnaserror` + `dotnet test tests/dotnet/Ikat.HeadlessTests/Ikat.HeadlessTests.csproj`，否则 CS 错误等到 push CI 甚至 tag 后才炸；且 **ABI 位型/宽度类扫荡（如 u32→u64）必须对顶层 Runtime 文件手 grep**：`uint` 局部接 FFI 返回值、`uint` 对 `RootSentinel`/`ulong.MaxValue` 类常量比较（CS0266+CS0652 组合是信号）——编译门罩不到这层（v0.0.12 曾因此二次移 tag）。**review 顶层 Runtime 是这层唯一的静态防线**：变量声明顺序（CS0841）、引用符号/重载逐项 grep 定义处核实存在性（CS0103 到 E: 机才炸，#109 review 实锤拦下三处 P0）。
