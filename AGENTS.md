...
- **本地 headless 工程不编 Runtime/ 层（编译门盲区，两次实锤）**——本地 `dotnet test` 编的是 Tests.Core 子集；headless 双工程（PublicApi/HeadlessTests）也只链 `Runtime/Public/ + Projection/ + Bindings/`，**顶层 Runtime 文件（IkatStageDriver/MirrorPool/FrameBlob/IkatInputCollector/NativeHostManager 等，引 UnityEngine）只有用户 Unity 才编**。改这些文件后收尾必跑两个 CI 工程：`dotnet build tests/dotnet/Ikat.PublicApi/Ikat.PublicApi.csproj -warnaserror` + `dotnet build tests/dotnet/Ikat.HeadlessTests/Ikat.HeadlessTests.csproj`，否则 CS 错误等到 push CI 甚至 tag 后才炸；且 **ABI 位型/宽度类扫荡（如 u32→u64）必须对顶层 Runtime 文件手 grep**：`uint` 局部接 FFI 返回值、`uint` 对 `RootSentinel`/`ulong.MaxValue` 类常量比较（CS0266+CS0652 组合是信号）——编译门罩不到这层（v0.0.12 曾因此二次移 tag）。
