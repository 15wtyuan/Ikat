...
namespace Yio.HeadlessTests
{
    ...
    public unsafe class TextFieldProjectionTests
    {
        ...
        // set_key_input 单条 KeyEvent(Enter down)。照 YioKeyEvent.cs 字段序：
        ...
        static void SetKeyDown(StageHandle* h, uint keyCode, byte modifiers = 0)
        {
            ...
            Native.yio_stage_set_key_input(h, &ev, 1);
        }
        ...
        static void SetTextInput(StageHandle* h, uint[] codepoints)
        {
            fixed (uint* cp = codepoints)
                Native.yio_stage_set_text_input(h, cp, (nuint)codepoints.Length);
        }

        // borrow_events → demuxer.Pump（复刻 YioHost.Step 的事件段）。
        static void PumpEvents(StageHandle* h, UIContext ctx)
        {
            nuint len = 0;
            byte* ptr = Native.yio_stage_borrow_events(h, &len);
