    ...
    public unsafe class VirtualizationTests
    {
        ...
        static int CountRenderNodesAfterTick(int itemCount)
        {
            ...
            try
            {
                ...
                // (mirrors YioHost.Step's ordering).
                for (int i = 0; i < 4; i++)
                    TickAndDrain(h, ctx);
        ...
        {
            // Same ordering as YioHost.Step: flush writes → drain pending binds → tick.
        ...
        static (float x, float y, float w, float h) GetLayoutRect(StageHandle* h, ulong node)
        {
            ...
            Native.yio_stage_get_node_layout_rect(h, node, &x, &y, &w, &hh);
            return (x, y, w, hh);
        }
        ...
        {
            int count = Native.yio_stage_get_child_count(h, ul);
            if (count == 0) return 0;
            ulong[] buf = new ulong[count];
            int written;
            fixed (ulong* bp = buf)
                written = Native.yio_stage_get_children(h, ul, bp, (nuint)count);
            for (int i = 0; i < written; i++)
            {
                byte kn = 0;
                Native.yio_stage_get_node_kind(h, buf[i], &kn);
        ...
        static void SetScrollPos(StageHandle* h, ulong pane, float y)
            => Native.yio_stage_set_scroll_pos(h, pane, 0.0f, y, animated: 0);

        static float GetScrollY(StageHandle* h, ulong pane)
        {
            float x, y;
            Native.yio_stage_get_scroll_pos(h, pane, &x, &y);
            return y;
        }
