...
{
    /// <summary>
    /// B3 harness smoke 测试：证明编码机能 P/Invoke 真 <c>yio_ffi_c.dll</c> 驱动 Stage，
    ...
    public unsafe class HarnessSmokeTests
    {
        ...
        public void GetNodeKindRoundTrip()
        {
            ...
            try
            {
                ...
                byte kind = 0xFF;
                int rc = Native.yio_stage_get_node_kind(
        ...
        /// <summary>
        /// 调 <c>yio_stage_create_root</c>。kind/css 用 UTF-8 字节（fixed 钉住 + ptr+len，
        /// 对齐 <c>YioStage.cs</c> 风格）。返 NodeId；0xFFFF_FFFF = 失败。
        /// </summary>
