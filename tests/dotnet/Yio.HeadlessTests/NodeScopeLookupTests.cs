...
namespace Yio.HeadlessTests
{
    ...
    public unsafe class NodeScopeLookupTests
    {
        ...
        /// **需要 T12 .dll 重编译**（含 yio_make_test_pkg + find_node_by_id_in_subtree FFI）。
        /// T12 后移除 Skip 即可运行。
        /// </summary>
        ...
        ///
        /// **需要 T12 .dll 重编译**（含 yio_make_test_pkg + yio_stage_find_node_by_id_in_subtree
        ...
        public unsafe void GetOnSlotHitsOwnBadgeNotOtherSlotsBadge()
        {
            ...
            try
            {
                ...
                {
                    pkgPtr = Native.yio_make_test_pkg(cp, (nuint)compName.Length, &outLen);
                    if (pkgPtr == null)
                        throw new InvalidOperationException("make_test_pkg returned null");
                    try
                    {
                        // 加载包 + 实例化 N 个 slot
                        int rc = Native.yio_stage_load_package(h,
                        ...
                        for (int i = 0; i < N; i++)
                        {
                            slots[i] = Native.yio_stage_instantiate(h,
                        ...
                        for (int i = 0; i < N; i++)
                        {
                            ...
                            ulong parent = Native.yio_node_parent(h, badge._id);
                    ...
                    {
                        Native.yio_bytes_free(pkgPtr, outLen);
                    }
                }
            }
            finally { StageHarness.Destroy(stage); }
            ...
            try
            {
                ...
                fixed (byte* cp = compName)
                {
                    ...
                    try
                    {
                        ...
                        ulong root = CreateRoot(stage, "div");
                        ulong slot0 = Native.yio_stage_instantiate(h,
        ...
        private static ulong CreateNode(IntPtr stage, string kind)
        {
            ...
            byte[] k = Encoding.UTF8.GetBytes(kind);
            fixed (byte* kp = k)
                return Native.yio_stage_create_node(h, kp, (nuint)k.Length, null, 0);
        }
            ...
            try
            {
                ...
                fixed (byte* cp = compName)
                {
                    ...
                    try
                    {
                        ...
                        ulong root = CreateRoot(stage, "div");
                        Container rootNode = (Container)ctx._registry.GetOrCreate(root);
                        ulong slot = Native.yio_stage_instantiate(h,
                            cp, (nuint)compName.Length, cp, (nuint)compName.Length);
