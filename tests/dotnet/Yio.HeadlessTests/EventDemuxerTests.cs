...
namespace Yio.HeadlessTests
{
    ...
    /// - <see cref="Yio.EventDemuxer"/> (<c>internal</c>) 翻译 raw YioEvent stream → typed struct → EventBus.Dispatch。
    ...
    public unsafe class EventDemuxerTests
    {
        ...
        public void DemuxTranslatesAndDispatches()
        {
            ...
            try
            {
                ...
                // 造 raw buffer 模拟 borrow_events 返回：单条 Click YioEvent
        ...
        public void KeyDownKeyAndModifiersAreFilled()
        {
            ...
            try
            {
                ...
                using (var buf = new NativeEventBuffer())
                {
                    // YioKeyCode.A = 97 (Unity YioKeyCode), modifiers = Shift(1)
                    buf.AddKeyDown(n._id, keyCode: 97, modifiers: 1);
                    ctx._eventDemuxer.Pump(buf.Ptr, buf.Count);
                }

                Assert.Equal(YioKeyCode.A, received.Key);
        ...
        public void KeyUpKeyAndModifiersFromRaw()
        {
            ...
            try
            {
                ...
                using (var buf = new NativeEventBuffer())
                {
                    // YioKeyCode.Enter = 13, modifiers = Ctrl(2) | Alt(4) = 6
                    buf.AddKeyUp(n._id, keyCode: 13, modifiers: 6);
                    ctx._eventDemuxer.Pump(buf.Ptr, buf.Count);
                }

                Assert.Equal(YioKeyCode.Enter, received.Key);
    ...
    /// 测试用——构造 YioEvent 条目写进 unmanaged 内存，透传给 EventDemuxer.Pump。
    /// Dispose 释放分配的内存。
    /// </summary>
