...
namespace Yio.HeadlessTests
{
    ...
    public unsafe class ControlStateProjectionTests
    {
        ...
        /// <summary>
        /// Node.Blur() 调 yio_stage_blur FFI 不抛（改前 throw NE）。
        ...
        public void uicontext_pick_hits_and_misses()
        {
            ...
            try
            {
                Node hit = ctx.Pick(new YioVector2(100f, 50f));
                Assert.NotNull(hit);
                ...
                Assert.Same(root, n);
                // 画布外 → null。
                Assert.Null(ctx.Pick(new YioVector2(999f, 999f)));
            }
            finally { StageHarness.Destroy(stage); }
        }
