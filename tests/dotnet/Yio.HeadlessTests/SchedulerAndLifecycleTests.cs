...
namespace Yio.HeadlessTests
{
    ...
    public unsafe class SchedulerAndLifecycleTests
    {
        ...
        static ulong CreateRoot(UIContext ctx)
        {
            ...
            fixed (byte* kp = k)
                return Native.yio_stage_create_root(H(ctx), kp, (nuint)k.Length, null, 0);
        }

        static void Tick(UIContext ctx, float dt = 0.016f) => Native.yio_stage_tick(H(ctx), dt);
        ...
        static (IntPtr stage, UIContext ctx, Container root, Dropdown sel) LoadDropdown()
        {
            ...
            Native.yio_stage_append_child(H(ctx), rootId, inst._id);
            Tick(ctx);
        ...
        public void CallAfterLayoutSeesSolvedGeometrySameFrame()
        {
            ...
            try
            {
                ...
                // 模拟 YioHost.Step 序：PumpLogic（帧头）→ flush → tick → PumpAfterLayout。
        ...
        public void OnUpdateStyleChangeSolvedSameFrame()
        {
            ...
            try
            {
                ...
                ctx.FlushPendingWrites();       // flush seam（YioHost.Step 在泵后自动调）
                Tick(ctx);
