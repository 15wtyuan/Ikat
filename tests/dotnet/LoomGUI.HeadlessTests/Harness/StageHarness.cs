...
{
    /// <summary>
    /// Headless 测试 harness：P/Invoke 真 <c>loomgui_ffi_c.dll</c> 驱动 Stage，
    ...
    internal static unsafe class StageHarness
    {
        ...
        {
            StageHandle* s = Native.loomgui_stage_new(w, h);
            if (s == null)
                throw new InvalidOperationException(
                    $"loomgui_stage_new({w}, {h}) returned null handle (dll load failed or Stage::new errored)");
        ...
        /// </summary>
        public static void Destroy(IntPtr stage) => Native.loomgui_stage_free((StageHandle*)stage.ToPointer());
    }
}
