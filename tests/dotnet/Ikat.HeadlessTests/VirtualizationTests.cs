    ...
    public unsafe class VirtualizationTests
    {
        ...
        static int CountRenderNodesAfterTick(int itemCount)
        {
            ...
            try
            {
                ...
                // (mirrors IkatHost.Step's ordering).
                for (int i = 0; i < 4; i++)
                    TickAndDrain(h, ctx);
        ...
        {
            // Same ordering as IkatHost.Step: flush writes → drain pending binds → tick.
        ...
        {
            nuint len = 0;
            byte* ptr = Native.ikat_stage_borrow_frame(h, &len);
            if (ptr == null || len < 12)
        ...
        static (float x, float y, float w, float h) GetLayoutRect(StageHandle* h, ulong node)
        {
            ...
            Native.ikat_stage_get_node_layout_rect(h, node, &x, &y, &w, &hh);
            return (x, y, w, hh);
        }
        ...
        static void SetScrollPos(StageHandle* h, ulong pane, float y)
            => Native.ikat_stage_set_scroll_pos(h, pane, 0.0f, y, animated: 0);

        static float GetScrollY(StageHandle* h, ulong pane)
        {
            float x, y;
            Native.ikat_stage_get_scroll_pos(h, pane, &x, &y);
            return y;
        }
