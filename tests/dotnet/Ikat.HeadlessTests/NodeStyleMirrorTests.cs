...
namespace Ikat.HeadlessTests
{
    ...
    ///
    /// 每条 Fact 验一条不变量：
    /// - getter 读镜像：写过的属性即时读回；未写过返 Unset 哨兵（Length/IkatColor/enum）。
    ...
    /// - IkatColor 8-hex round-trip（#rrggbbaa）：验 parse_color fix（commit 4aa8b3c）+ seam 全链通。
    ...
    public unsafe class NodeStyleMirrorTests
    {
        ...
        /// <summary>
        /// 未写过的 IkatColor / enum 属性 getter 返 Unset 哨兵。
        /// </summary>
        [Fact]
        ...
        /// <summary>
        /// IkatColor 8-hex round-trip：BackgroundColor=red(1,0,0,1) → CssValueConvert 出 #ff0000ff →
        ...
        public void StyleColorRoundTrips8Hex()
        {
            ...
            {
                Node n = ctx._registry.GetOrCreate(CreateRoot(stage, "div"));
                n.Style.BackgroundColor = new IkatColor(1f, 0f, 0f, 1f);   // → #ff0000ff
                Tick(stage, ctx);
        ...
        /// <summary>
        /// IkatColor 半透明 round-trip：BackgroundColor=(1,1,1,0.5) → #ffffff80 → 解析回 (1,1,1,~0.5)。
        ...
        public void StyleColorRoundTripsAlpha()
        {
            ...
            {
                Node n = ctx._registry.GetOrCreate(CreateRoot(stage, "div"));
                n.Style.BackgroundColor = new IkatColor(1f, 1f, 1f, 0.5f);
                Tick(stage, ctx);
        ...
        /// <summary>
        /// IkatColor Unset 哨兵 setter → unset → 下帧 computed_style.bg_present=0（回落 base 无 bg）。
        /// 验 IkatColor 的 Unset 哨兵分支（IkatColor.IsUnset → Unset(prop)）。
        /// </summary>
        [Fact]
        public void StyleColorUnsetClearsBackground()
        {
            ...
            {
                Node n = ctx._registry.GetOrCreate(CreateRoot(stage, "div"));
                n.Style.BackgroundColor = new IkatColor(1f, 0f, 0f, 1f);
                Tick(stage, ctx);
                Assert.Equal(1, GetComputedStyle(stage, n._id).bg_present);

                n.Style.BackgroundColor = IkatColor.Unset;   // IsUnset=true → Unset(prop)
                Tick(stage, ctx);
        ...
        public void StyleMultiplePropsCoexist()
        {
            ...
            try
            {
                Node n = ctx._registry.GetOrCreate(CreateRoot(stage, "div"));
                n.Style.Width = Length.Px(100);
                n.Style.BackgroundColor = new IkatColor(0f, 1f, 0f, 1f);
        ...
        private static (float x, float y, float w, float h) GetLayoutRect(IntPtr stage, ulong id)
        {
            ...
            Native.ikat_stage_get_node_layout_rect(h, id, &x, &y, &w, &hh);
            return (x, y, w, hh);
        }
        ...
        private static ComputedNodeStyleRepr GetComputedStyle(IntPtr stage, ulong id)
        {
            ...
            int rc = Native.ikat_stage_get_node_computed_style(h, id, &repr);
            if (rc != 0)
