    ...
    public unsafe class UiContextCreationTests
    {
        ...
        static Container InitRoot(UIContext ctx, string kind = "div", string css = "")
        {
            ...
            fixed (byte* kp = k, cp = c)
                id = Native.ikat_stage_create_root(h, kp, (nuint)k.Length, cp, (nuint)c.Length);
            if (id == RootSentinel)
        ...
        public void CreateContainerWhitelist()
        {
            ...
            try
            {
                ...
                StageHandle* h = (StageHandle*)stage.ToPointer();
                byte kind = 0xFF;
                int rc = Native.ikat_stage_get_node_kind(h, c._id, &kind);
                Assert.Equal(0, rc);
        ...
        public void CreateAbsolutePanelWhitelist()
        {
            ...
            try
            {
                ...
                StageHandle* h = (StageHandle*)stage.ToPointer();
                byte kind = 0xFF;
                int rc = Native.ikat_stage_get_node_kind(h, p._id, &kind);
                Assert.Equal(0, rc);
        ...
        public void CreateTextNodeWhitelist()
        {
            ...
            try
            {
                ...
                StageHandle* h = (StageHandle*)stage.ToPointer();
                byte kind = 0xFF;
                int rc = Native.ikat_stage_get_node_kind(h, tn._id, &kind);
                Assert.Equal(0, rc);
        ...
        public void CreateImageWhitelist()
        {
            ...
            try
            {
                ...
                StageHandle* h = (StageHandle*)stage.ToPointer();
                byte kind = 0xFF;
                int rc = Native.ikat_stage_get_node_kind(h, img._id, &kind);
                Assert.Equal(0, rc);
        ...
        public void FocusedNodeAfterRequestFocus()
        {
            ...
            try
            {
                ...
                Native.ikat_stage_request_focus(h, c._id);
                Native.ikat_stage_tick(h, 0.016f);   // consume pending_focus_request
        ...
        // （原 PickThrowsNe 已改——Pick 已接通 ikat_stage_hit_test。空 stage（无 scene）
        ...
        public void PickOnEmptyStageThrowsInvalidOp()
        {
            ...
            try
            {
                Assert.Throws<InvalidOperationException>(() => ctx.Pick(new IkatVector2(100, 100)));
            }
            finally { StageHarness.Destroy(stage); }
        }
