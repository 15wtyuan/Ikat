...
namespace Ikat.HeadlessTests
{
    ...
    /// - <see cref="Ikat.EventBus"/>(<c>internal</c>) 持订阅表 <c>Dictionary&lt;(nodeId,eventType,capture), List&lt;HandlerEntry&gt;&gt;</c>。
    ...
    /// - <see cref="Ikat.EventBus.Dispatch{T}"/> 走 capture/bubble 路由 + StopPropagation halt + once auto-remove。
    ///
    /// D2 自己构造 typed event 喂 Dispatch（模拟 D3 demux 翻译 raw IkatEvent → typed struct 后调 Dispatch）。
