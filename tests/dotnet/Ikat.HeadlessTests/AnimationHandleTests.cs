...
namespace Ikat.HeadlessTests
{
    ...
    ///
    /// fixture：animation.pkg.bin（@keyframes fade + hookanim，后者带 @ikat-hook "half" 锚在 50% stop），
    ...
    public unsafe class AnimationHandleTests
    {
        ...
        /// <summary>
        /// @ikat-hook 锚点：fixture hookanim 的 50% stop 带 hook="half"（注释挂前一个 stop，
        /// 故 50%{}/* @ikat-hook half */ to{} → half 锚 50%）。player 跨 50% 时 emit HOOK，
        ...
        {
            StageHandle* h = (StageHandle*)stage.ToPointer();
            Native.ikat_stage_tick(h, dt);
            nuint evLen = 0;
            byte* evPtr = Native.ikat_stage_borrow_events(h, &evLen);
