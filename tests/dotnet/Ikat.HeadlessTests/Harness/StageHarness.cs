...
{
    /// <summary>
    /// Headless 测试 harness：P/Invoke 真 <c>ikat_ffi_c.dll</c> 驱动 Stage，
    ...
    internal static unsafe class StageHarness
    {
        ...
        {
            StageHandle* s = Native.ikat_stage_new(w, h);
            if (s == null)
                throw new InvalidOperationException(
                    $"ikat_stage_new({w}, {h}) returned null handle (dll load failed or Stage::new errored)");
        ...
        /// </summary>
        public static void Destroy(IntPtr stage) => Native.ikat_stage_free((StageHandle*)stage.ToPointer());
    }
}
