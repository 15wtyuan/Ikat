...
namespace Ikat.HeadlessTests
{
    ...
    public unsafe class ControlStateProjectionTests
    {
        ...
        /// <summary>
        /// Node.Blur() 调 ikat_stage_blur FFI 不抛（改前 throw NE）。
        ...
        public void uicontext_pick_hits_and_misses()
        {
            ...
            try
            {
                Node hit = ctx.Pick(new IkatVector2(100f, 50f));
                Assert.NotNull(hit);
                ...
                Assert.Same(root, n);
                // 画布外 → null。
                Assert.Null(ctx.Pick(new IkatVector2(999f, 999f)));
            }
            finally { StageHarness.Destroy(stage); }
        }
        ...
        public void node_touchable_roundtrip_and_pick()
        {
            ...
            try
            {
                ...
                Node before = ctx.Pick(new IkatVector2(100f, 50f));
                Assert.Same(root, before);
                root.Touchable = false;
                Assert.False(root.Touchable);
                // 外层 scene root（fixture harness 造的包装 div）仍可命中——本节点被跳过。
                Assert.NotSame(root, ctx.Pick(new IkatVector2(100f, 50f)));
                root.Touchable = true;
                Assert.True(root.Touchable);
                Assert.Same(root, ctx.Pick(new IkatVector2(100f, 50f)));
            }
            finally { StageHarness.Destroy(stage); }
        }
